...
class TestSparseDataFrame(SharedWithSparse):
    ...
    def test_constructor_ndarray(self, float_frame):
        ...
        pytest.raises(TypeError, float_frame.reindex, columns=["A"], level=1)
    ...
    def test_setitem(
        ...
        def _check_frame(frame, orig):
            ...
            pytest.raises(Exception, frame.__setitem__, "foo", np.random.randn(N - 1))
    ...
    def test_set_columns(self, float_frame):
        ...
        pytest.raises(
            Exception, setattr, float_frame, "columns", float_frame.columns[:-1]
        )

    def test_set_index(self, float_frame):
        float_frame.index = float_frame.index
        pytest.raises(Exception, setattr, float_frame, "index", float_frame.index[:-1])
