...
class TestSparseSeries(SharedWithSparse):
    ...
    def test_constructor(self):
        ...
        tm.assert_numpy_array_equal(
            self.zbseries.values.values, self.bseries.to_dense().fillna(0).values
        )
    ...
    def test_getitem(self):
        ...
        # exception handling
        pytest.raises(Exception, self.bseries.__getitem__, len(self.bseries) + 1)

        # index not contained
        pytest.raises(
            Exception, self.btseries.__getitem__, self.btseries.index[-1] + BDay()
        )
    ...
    def test_take(self):
        def _compare_with_dense(sp):
            ...
            def _compare(idx):
                ...
                tm.assert_almost_equal(dense_result, sparse_result.values.values)
        ...
        pytest.raises(Exception, self.bseries.take, [0, len(self.bseries) + 1])
