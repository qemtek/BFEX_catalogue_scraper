...
class TestMerge(object):
    ...
    def test_index_and_on_parameters_confusion(self):
        msg = (
            "right_index parameter must be of type bool, not" r" <(class|type) 'list'>"
        )
        with pytest.raises(ValueError, match=msg):
            merge(
                self.df,
        ...
        msg = "left_index parameter must be of type bool, not " r"<(class|type) 'list'>"
        with pytest.raises(ValueError, match=msg):
            merge(
                self.df,
                self.df2,
    ...
    def test_intelligently_handle_join_key(self):
        ...
        left = DataFrame(
            {"key": [1, 1, 2, 2, 3], "value": lrange(5)}, columns=["value", "key"]
        )
        right = DataFrame({"key": [1, 1, 2, 3, 4, 5], "rvalue": lrange(6)})
    ...
    def test_overlapping_columns_error_message(self):
        ...
        msg = (
            r"Data columns not unique: Index\(\[u?'foo', u?'foo'\],"
            r" dtype='object'\)"
        )
        with pytest.raises(MergeError, match=msg):
            merge(df, df2)
    ...
    def test_merge_on_index_with_more_values(self, how):
        ...
        df1 = pd.DataFrame([[1, 2], [2, 4], [3, 6], [4, 8]], columns=["a", "b"])
        df2 = pd.DataFrame([[3, 30], [4, 40]], columns=["a", "c"])
        df1.set_index("a", drop=False, inplace=True)
        df2.set_index("a", inplace=True)
        result = pd.merge(df1, df2, left_index=True, right_on="a", how=how)
        expected = pd.DataFrame(
            [[1, 2, np.nan], [2, 4, np.nan], [3, 6, 30.0], [4, 8, 40.0]],
            columns=["a", "b", "c"],
        )
