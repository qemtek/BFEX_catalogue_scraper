...
class TestAppend(ConcatenateBase):
    ...
    def test_append_different_columns_types_raises(
        ...
        msg = (
            "the other index needs to be an IntervalIndex too, but was"
            r" type {}|"
            r"object of type '(int|long|float|Timestamp)' has no len\(\)|"
            "Expected tuple, got str"
        )
        with pytest.raises(
            TypeError, match=msg.format(index_can_append.__class__.__name__)
        ):
            df.append(ser)
...
class TestConcatenate(ConcatenateBase):
    ...
    def test_concat_mixed_objs(self):
        ...
        with catch_warnings(record=True):
            ...
            panel = tm.makePanel()
            msg = "cannot concatenate unaligned mixed dimensional NDFrame" " objects"
            with pytest.raises(ValueError, match=msg):
    ...
    def test_panel_concat_buglet(self, sort):
        ...
        panel2 = panel2.rename(major_axis={x: "%s_1" % x for x in panel2.major_axis})
...
"""
    ...
    def test_concat_empty_series(self):
        ...
        res = pd.concat([s1, s2], axis=1)
        exp = pd.DataFrame(
            {"x": [1, 2, 3], 0: [np.nan, np.nan, np.nan]}, columns=["x", 0]
        )
        tm.assert_frame_equal(res, exp)
