...
class TestIndex(Base):
    ...
    def setup_method(self, method):
        self.indices = dict(
            ...
            empty=Index([]),
            tuples=MultiIndex.from_tuples(lzip(["foo", "bar", "baz"], [1, 2, 3])),
    ...
    def test_constructor_dtypes_datetime(self, tz_naive_fixture, attr, utc, klass):
        ...
        if (
            tz_naive_fixture
            and attr == "asi8"
            and str(tz_naive_fixture) not in ("UTC", "tzutc()")
        ):
        ...
        with tm.assert_produces_warning(ex_warn, check_stacklevel=False, clear=modules):
    ...
    def test_slice_ints_with_floats_raises(self):
        ...
        pytest.raises(TypeError, lambda: index.slice_locs(5.0, 10.0))
        pytest.raises(TypeError, lambda: index.slice_locs(4.5, 10.5))

        index2 = index[::-1]
        pytest.raises(TypeError, lambda: index2.slice_locs(8.5, 1.5), (2, 6))
        pytest.raises(TypeError, lambda: index2.slice_locs(10.5, -1), (0, n))
    ...
    @pytest.mark.parametrize(
        "index",
        [Index(["qux", "baz", "foo", "bar"]), Float64Index([1.0, 2.0, 3.0, 4.0])],
    )
...
Index([u'a', u'bb', u'ccc', u'a', u'bb', u'ccc', u'a', u'bb', u'ccc', u'a',
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                pd.Index([u"あ", u"いい", u"ううう"]),
                (u"Index(['あ', 'いい', 'ううう'], " u"dtype='object')"),
            ),
            # multiple lines
            (
                pd.Index([u"あ", u"いい", u"ううう"] * 10),
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                pd.Index([u"あ", u"いい", u"ううう"]),
                (u"Index([u'あ', u'いい', u'ううう'], " u"dtype='object')"),
            ),
            # multiple lines
            (
                pd.Index([u"あ", u"いい", u"ううう"] * 10),
    ...
    def test_string_index_repr_with_unicode_option_compat(self, index, expected):
        # Enable Unicode option -----------------------------------------
        with cf.option_context("display.unicode.east_asian_width", True):
            result = unicode(index)  # noqa
            assert result == expected
...
class TestMixedIntIndex(Base):
    ...
    @pytest.mark.parametrize(
        "func,compat_func",
        [(str, text_type), (bytes, str)],  # unicode string  # byte string
    )
