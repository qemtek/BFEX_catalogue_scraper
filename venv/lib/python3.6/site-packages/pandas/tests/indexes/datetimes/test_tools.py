...
class TestTimeConversionFormats(object):
    ...
    @pytest.mark.parametrize("box,const", [[True, pd.Index], [False, np.array]])
    ...
    def test_to_datetime_parse_tzname_or_tzoffset(
        self, box, const, fmt, dates, expected_dates
    ):
        # GH 13486
...
class TestToDatetime(object):
    ...
    def test_to_datetime_array_of_dt64s(self, cache):
        ...
        pytest.raises(ValueError, pd.to_datetime, dts_with_oob, errors="raise")

        tm.assert_numpy_array_equal(
            pd.to_datetime(dts_with_oob, box=False, errors="coerce", cache=cache),
            np.array(
                [
        ...
        tm.assert_numpy_array_equal(
            pd.to_datetime(dts_with_oob, box=False, errors="ignore", cache=cache),
            np.array([dt.item() for dt in dts_with_oob], dtype="O"),
        )
    ...
    def test_to_datetime_cache(self, utc, format, box, constructor):
        ...
        result = pd.to_datetime(data, utc=utc, format=format, box=box, cache=True)
        expected = pd.to_datetime(data, utc=utc, format=format, box=box, cache=False)
        if box:
            tm.assert_index_equal(result, expected)
        else:
...
class TestToDatetimeUnit(object):
    ...
    def test_unit_ignore_keeps_name(self, cache):
        ...
        result = pd.to_datetime(
            expected, errors="ignore", box=True, unit="s", cache=cache
        )
        tm.assert_index_equal(result, expected)
    ...
    def test_to_datetime_errors_ignore_utc_true(self):
        # GH 23758
        result = pd.to_datetime([1], unit="s", box=True, utc=True, errors="ignore")
...
class TestToDatetimeMisc(object):
    ...
    def test_to_datetime_with_space_in_series(self, cache):
        ...
        pytest.raises(ValueError, lambda: to_datetime(s, errors="raise", cache=cache))
    ...
    def test_to_datetime_with_apply(self, cache):
        ...
        pytest.raises(
            ValueError,
            lambda: pd.to_datetime(td, format="%b %y", errors="raise", cache=cache),
        )
        pytest.raises(
            ValueError,
            lambda: td.apply(
    ...
    @pytest.mark.parametrize("cache", [True, False])
    @pytest.mark.parametrize("box, klass", [[True, Index], [False, np.array]])
    def test_to_datetime_unprocessable_input(self, cache, box, klass):
        ...
        tm.assert_equal(result, expected)
        pytest.raises(
            TypeError, to_datetime, [1, "1"], errors="raise", cache=cache, box=box
        )
    ...
    def test_string_na_nat_conversion(self, cache):
        ...
        pytest.raises(
            ValueError, lambda: to_datetime(malformed, errors="raise", cache=cache)
        )
        ...
        pytest.raises(ValueError, to_datetime, malformed, errors="raise", cache=cache)
...
class TestDaysInMonth(object):
    ...
    def test_day_not_in_month_raise(self, cache):
        pytest.raises(
            ValueError, to_datetime, "2015-02-29", errors="raise", cache=cache
        )
        pytest.raises(
            ValueError,
            to_datetime,
