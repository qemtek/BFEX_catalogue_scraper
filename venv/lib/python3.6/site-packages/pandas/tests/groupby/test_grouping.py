...
class TestGrouping:
    ...
    def test_groupby_categorical_index_and_columns(self, observed):
        ...
        if observed:
            ...
            expected_columns = CategoricalIndex(
                ["A", "B"], categories=categories, ordered=True
            )
        else:
            expected_columns = CategoricalIndex(
    ...
    def test_groupby_level_index_names(self):
        ...
        df = DataFrame({"exp": ["A"] * 3 + ["B"] * 3, "var1": lrange(6)}).set_index(
            "exp"
        )
...
class TestIteration:
    ...
    def test_multi_iter_panel(self):
        wp = tm.makePanel()
        grouped = wp.groupby([lambda x: x.month, lambda x: x.weekday()], axis=1)

        for (month, wd), group in grouped:
            exp_axis = [
                x for x in wp.major_axis if x.month == month and x.weekday() == wd
            ]
