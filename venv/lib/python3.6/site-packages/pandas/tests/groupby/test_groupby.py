...
def test_basic_regression():
    ...
    groupings = Series(groupings, lrange(0, len(groupings))) * 10.0
...
def test_groupby_multiple_columns(df, op):
    ...
    expected = {k: DataFrame(v) for k, v in compat.iteritems(expected)}
...
def test_panel_groupby():
    panel = tm.makePanel()
    tm.add_nans(panel)
    grouped = panel.groupby({"ItemA": 0, "ItemB": 0, "ItemC": 1}, axis="items")
    agged = grouped.mean()
    ...
    grouped = panel.groupby({"A": 0, "B": 0, "C": 1, "D": 1}, axis="minor")
    agged = grouped.mean()
