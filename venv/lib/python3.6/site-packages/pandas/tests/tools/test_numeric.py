...
class TestToNumeric(object):
    ...
    def test_numeric_lists_and_arrays(self):
        ...
        df = pd.DataFrame(
            dict(a=[[decimal.Decimal(3.14), 1.0], decimal.Decimal(1.6), 0.1])
        )
        df["a"] = df["a"].apply(to_numeric)
        expected = pd.DataFrame(dict(a=[[3.14, 1.0], 1.6, 0.1]))
        tm.assert_frame_equal(df, expected)

        df = pd.DataFrame(dict(a=[np.array([decimal.Decimal(3.14), 1.0]), 0.1]))
        df["a"] = df["a"].apply(to_numeric)
        expected = pd.DataFrame(dict(a=[[3.14, 1.0], 0.1]))
        tm.assert_frame_equal(df, expected)
    ...
    def test_numeric_dtypes(self):
        ...
        idx = pd.Index([1.0, np.nan, 3.0, np.nan], name="xxx")
        res = pd.to_numeric(idx)
        tm.assert_index_equal(res, idx)
    ...
    def test_datetime_like(self, tz_naive_fixture):
        idx = pd.date_range("20130101", periods=3, tz=tz_naive_fixture, name="xxx")
        res = pd.to_numeric(idx)
    ...
    def test_ignore_downcast_invalid_data(self):
        ...
        res = pd.to_numeric(data, errors="ignore", downcast="unsigned")
        tm.assert_numpy_array_equal(res, expected)
    ...
    def test_ignore_downcast_cannot_convert_float(self, data, expected, downcast):
    ...
    def test_coerce_uint64_conflict(self):
        ...
        expected = pd.Series(
            [200, 300, np.nan, np.nan, 30000000000000000000], dtype=float, name="a"
        )
        result = to_numeric(df["a"], errors="coerce")
        tm.assert_series_equal(result, expected)

        s = pd.Series(["12345678901234567890", "1234567890", "ITEM"])
        expected = pd.Series([12345678901234567890, 1234567890, np.nan], dtype=float)
