...
class TestCompat(object):
    ...
    def test_query_numexpr(self):
        ...
        if _NUMEXPR_INSTALLED:
            ...
            pytest.raises(ImportError, lambda: df.query("A>0", engine="numexpr"))
            pytest.raises(ImportError, lambda: df.eval("A+1", engine="numexpr"))
...
class TestDataFrameQueryNumExprPandas(object):
    ...
    def test_query_syntax_error(self):
        engine, parser = self.engine, self.parser
        df = DataFrame({"i": lrange(10), "+": lrange(3, 13), "r": lrange(4, 14)})
