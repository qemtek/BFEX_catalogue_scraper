...
class TestDataFrameToCSV(TestData):
    ...
    def test_to_csv_from_csv1(self):
        ...
        with ensure_clean("__tmp_to_csv_from_csv1__") as path:
            ...
            # corner case
            dm = DataFrame(
                {"s1": Series(lrange(3), lrange(3)), "s2": Series(lrange(2), lrange(2))}
            )
            dm.to_csv(path)
    ...
    def test_to_csv_from_csv2(self):
        ...
        with ensure_clean("__tmp_to_csv_from_csv2__") as path:
            ...
            pytest.raises(ValueError, self.frame2.to_csv, path, header=["AA", "X"])
            ...
            if r_dtype:
                if r_dtype == "u":  # unicode
                    ...
                    recons.index = np.array(
                        lmap(Timestamp, recons.index), dtype=r_dtype
                    )
                    df.index = np.array(lmap(Timestamp, df.index), dtype=r_dtype)
                elif r_dtype == "p":
                    r_dtype = "O"
                    recons.index = np.array(
                        list(map(Timestamp, to_datetime(recons.index))), dtype=r_dtype
                    )
                    df.index = np.array(
            ...
            if c_dtype:
                if c_dtype == "u":
                    ...
                    recons.columns = np.array(
                        lmap(_to_uni, recons.columns), dtype=c_dtype
                    )
                    df.columns = np.array(lmap(_to_uni, df.columns), dtype=c_dtype)
                elif c_dtype == "dt":
                    c_dtype = "O"
                    recons.columns = np.array(
                        lmap(Timestamp, recons.columns), dtype=c_dtype
                    )
                    df.columns = np.array(lmap(Timestamp, df.columns), dtype=c_dtype)
                elif c_dtype == "p":
                    c_dtype = "O"
                    recons.columns = np.array(
                        lmap(Timestamp, to_datetime(recons.columns)), dtype=c_dtype
                    )
                    df.columns = np.array(
                        lmap(Timestamp, df.columns.to_timestamp()), dtype=c_dtype
                    )
                else:
    ...
    def test_to_csv_dups_cols(self):
        ...
        df = DataFrame(
            np.random.randn(1000, 30), columns=lrange(15) + lrange(15), dtype="float64"
        )
        ...
        df_dt = DataFrame(
            Timestamp("20010101"), index=df_float.index, columns=lrange(3)
        )
        df = pd.concat(
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (DataFrame(5 * [[123, u"你好", u"世界"]], columns=["X", "Y", "Z"]), "gb2312"),
            (
                DataFrame(5 * [[123, u"Γειά σου", u"Κόσμε"]], columns=["X", "Y", "Z"]),
                "cp737",
            ),
        ],
    )
