...
class TestDataFrameConstructors(TestData):
    ...
    def test_constructor_cast_failure(self):
        ...
        # this is not ok
        pytest.raises(ValueError, df.__setitem__, tuple(["test"]), np.ones((4, 2)))
    ...
    def test_constructor_subclass_dict(self):
        ...
        df = DataFrame(data)
        refdf = DataFrame(
            {col: dict(compat.iteritems(val)) for col, val in compat.iteritems(data)}
        )
    ...
    @pytest.mark.skipif(PY2 and _np_version_under1p13, reason="old numpy & py2")
    ...
    def test_constructor_column_duplicates(self):
        ...
        pytest.raises(
            ValueError, DataFrame.from_dict, OrderedDict([("b", 8), ("a", 5), ("a", 6)])
        )
    ...
    def test_constructor_for_list_with_dtypes(self):
        ...
        df = DataFrame({"a": 1.0}, index=lrange(3))
        result = df.get_dtype_counts()
        expected = Series({"float64": 1})
        tm.assert_series_equal(result, expected)
    ...
    def test_constructor_categorical(self):
        ...
        pytest.raises(
            ValueError,
            lambda: DataFrame([Categorical(list("abc")), Categorical(list("abdefg"))]),
        )

        # ndim > 1
        pytest.raises(
            NotImplementedError, lambda: Categorical(np.array([list("abcd")]))
        )
