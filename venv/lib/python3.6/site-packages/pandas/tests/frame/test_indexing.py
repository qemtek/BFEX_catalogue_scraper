...
class TestDataFrameIndexing(TestData):
    ...
    def test_getattr(self):
        ...
        pytest.raises(AttributeError, getattr, self.frame, "NONEXISTENT_NAME")
    ...
    def test_setitem_ambig(self):
        ...
        coercable_series = Series([Decimal(1) for _ in range(3)], index=lrange(3))
    ...
    def test_setitem_fancy_2d(self):
        ...
        frame = DataFrame(lzip([2, 3, 9, 6, 7], [np.nan] * 5), columns=["a", "b"])
        lst = [100]
        lst.extend([np.nan] * 4)
        expected = DataFrame(lzip([100, 3, 9, 6, 7], lst), columns=["a", "b"])
        frame[frame["a"] == 2] = 100
        assert_frame_equal(frame, expected)
    ...
    def test_fancy_index_int_labels_exceptions(self):
        ...
        with catch_warnings(record=True):
            ...
            pytest.raises(KeyError, df.ix.__setitem__, ([0, 1, 2], [2, 3, 4]), 5)

            # try to set indices not contained in frame
            pytest.raises(KeyError, self.frame.ix.__setitem__, ["foo", "bar", "baz"], 1)
            pytest.raises(
                KeyError, self.frame.ix.__setitem__, (slice(None, None), ["E"]), 1
            )
    ...
    def test_set_value_with_index_dtype_change(self):
        df_orig = DataFrame(np.random.randn(3, 3), index=lrange(3), columns=list("ABC"))
    ...
    def test_xs_view(self):
        ...
        dm = DataFrame(
            np.arange(20.0).reshape(4, 5), index=lrange(4), columns=lrange(5)
        )
    ...
    def test_where(self):
        ...
        def _safe_add(df):
            ...
            return DataFrame(
                dict(
                    (c, s + 1) if is_ok(s) else (c, s) for c, s in compat.iteritems(df)
                )
            )
