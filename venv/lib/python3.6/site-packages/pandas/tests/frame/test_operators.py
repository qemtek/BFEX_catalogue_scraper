...
class TestDataFrameOperators(TestData):
    ...
    def test_combineSeries(self):
        ...
        expected = DataFrame(
            np.nan, index=self.tsframe.index, columns=self.tsframe.columns
        )
        assert_frame_equal(result, expected)
    ...
    def test_comparisons(self):
        ...
        def test_comp(func):
            ...
            tm.assert_numpy_array_equal(
                result2.values, func(self.simple.values, row.values)
            )
    ...
    def test_combine_generic(self):
        ...
        exp = (
            self.frame.loc[self.frame.index[:-5], ["A", "B", "C"]].reindex_like(chunk)
            * 2
        )
