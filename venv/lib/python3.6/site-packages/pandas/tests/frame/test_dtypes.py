...
class TestDataFrameDataTypes(TestData):
    ...
    def test_select_dtypes_include_using_list_like(self):
        ...
        pytest.raises(NotImplementedError, lambda: df.select_dtypes(include=["period"]))
    ...
    def test_select_dtypes_exclude_using_scalars(self):
        ...
        pytest.raises(NotImplementedError, lambda: df.select_dtypes(exclude="period"))
    ...
    @pytest.mark.parametrize(
        "dtype",
        [str, "str", np.string_, "S1", "unicode", np.unicode_, "U1", compat.text_type],
    )
    ...
    def test_dtypes_gh8722(self):
        ...
        expected = Series(
            {k: v.dtype for k, v in compat.iteritems(self.mixed_frame)},
            index=result.index,
        )
        assert_series_equal(result, expected)
    ...
    def test_astype(self):
        ...
        def _check_cast(df, v):
            assert list({s.dtype.name for _, s in compat.iteritems(df)})[0] == v
    ...
    def test_astype_str(self, text_dtype):
        ...
        expected = DataFrame(
            {
                "a": list(
                    map(text_dtype, map(lambda x: Timestamp(x)._date_repr, a._values))
                ),
...
class TestDataFrameDatetimeWithTZ(TestData):
    ...
    def test_astype(self):
        ...
        assert_frame_equal(
            result,
            DataFrame(expected, index=self.tzframe.index, columns=self.tzframe.columns),
        )
