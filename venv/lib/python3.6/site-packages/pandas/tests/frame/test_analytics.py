...
class TestDataFrameAnalytics:
    ...
    def test_corr_invalid_method(self):
        ...
        msg = "method must be either 'pearson', 'spearman', " "or 'kendall'"
    ...
    def test_nunique(self, float_frame_with_na, float_frame, float_string_frame):
    ...
    def test_sum(
        self, float_frame_with_na, mixed_float_frame, float_frame, float_string_frame
    ):
        assert_stat_op_api(
    ...
    def test_product(self, float_frame_with_na, float_frame, float_string_frame):
        assert_stat_op_calc("product", np.prod, float_frame_with_na)
        assert_stat_op_api("product", float_frame, float_string_frame)

    # TODO: Ensure warning isn't emitted in the first place
    @pytest.mark.filterwarnings("ignore:All-NaN:RuntimeWarning")
    def test_median(self, float_frame_with_na, float_frame, float_string_frame):
        def wrapper(x):
            if isna(x).any():
                return np.nan
            return np.median(x)
    ...
    def test_min(self, float_frame_with_na, int_frame, float_frame, float_string_frame):
        with warnings.catch_warnings(record=True):
            warnings.simplefilter("ignore", RuntimeWarning)
            assert_stat_op_calc("min", np.min, float_frame_with_na, check_dates=True)
    ...
    def test_max(self, float_frame_with_na, int_frame, float_frame, float_string_frame):
        with warnings.catch_warnings(record=True):
            warnings.simplefilter("ignore", RuntimeWarning)
            assert_stat_op_calc("max", np.max, float_frame_with_na, check_dates=True)
    ...
    def test_var_std(
        self, float_frame_with_na, datetime_frame, float_frame, float_string_frame
    ):
    ...
    def test_numeric_only_flag(self, meth):
        ...
        pytest.raises(TypeError, lambda: getattr(df1, meth)(axis=1, numeric_only=False))
        pytest.raises(TypeError, lambda: getattr(df2, meth)(axis=1, numeric_only=False))
    ...
    def test_cumsum_corner(self):
        dm = DataFrame(np.arange(20).reshape(4, 5), index=lrange(4), columns=lrange(5))
        # ?(wesm)
    ...
    def test_built_in_round(self):
        if not compat.PY3:
            pytest.skip("build in round cannot be overridden " "prior to Python 3")
    ...
    def test_dot(self):
        ...
        # unaligned
        df = DataFrame(np.random.randn(3, 4), index=[1, 2, 3], columns=lrange(4))
        df2 = DataFrame(np.random.randn(5, 3), index=lrange(5), columns=[1, 2, 3])

        with pytest.raises(ValueError, match="aligned"):
            df.dot(df2)

    @pytest.mark.skipif(not PY35, reason="matmul supported for Python>=3.5")
