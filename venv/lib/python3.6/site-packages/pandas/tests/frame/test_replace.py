...
class TestDataFrameReplace(TestData):
    ...
    def test_regex_replace_dict_mixed(self):
        ...
        expec = DataFrame(
            {"a": mix["a"], "b": ["a", "b", np.nan, np.nan], "c": mix["c"]}
        )
        assert_frame_equal(res, expec)
        ...
        expec = DataFrame({"a": mix["a"], "b": [np.nan, "b", ".", "."], "c": mix["c"]})
    ...
    def test_regex_replace_regex_list_to_numeric(self):
        ...
        expec = DataFrame(
            {"a": mix["a"], "b": ["a", 0, 0, 0], "c": ["a", 0, np.nan, "d"]}
        )
        assert_frame_equal(res, expec)
        assert_frame_equal(res2, expec)
        assert_frame_equal(res3, expec)
    ...
    def test_replace_input_formats_listlike(self):
        ...
        expected = {k: v.replace(to_rep[k], values[k]) for k, v in compat.iteritems(df)}
        ...
        expected = {k: v.replace(np.nan, values[k]) for k, v in compat.iteritems(df)}
        assert_frame_equal(filled, DataFrame(expected))
    ...
    def test_replace_input_formats_scalar(self):
        ...
        expected = {k: v.replace(to_rep[k], 0) for k, v in compat.iteritems(df)}
        assert_frame_equal(filled, DataFrame(expected))
