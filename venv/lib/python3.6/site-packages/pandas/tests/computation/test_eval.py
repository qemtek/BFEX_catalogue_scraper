...
from pandas.core.computation.ops import _arith_ops_syms
from pandas.core.computation.ops import _binary_math_ops
from pandas.core.computation.ops import _binary_ops_dict
from pandas.core.computation.ops import _bool_ops_syms
from pandas.core.computation.ops import _special_case_arith_ops_syms
...
def _is_py3_complex_incompat(result, expected):
    return (
        PY3 and isinstance(expected, (complex, np.complexfloating)) and np.isnan(result)
    )
...
class TestEvalNumexprPandas(object):
    ...
    def test_complex_cmp_ops(self):
        cmp_ops = ("!=", "==", "<=", ">=", "<", ">")
        cmp2_ops = (">", "<")
        for lhs, cmp1, rhs, binop, cmp2 in product(
            self.lhses, cmp_ops, self.rhses, self.bin_ops, cmp2_ops
        ):
    ...
    def check_complex_cmp_op(self, lhs, cmp1, rhs, binop, cmp2):
        ...
        scalar_with_in_notin = is_scalar(rhs) and (
            cmp1 in skip_these or cmp2 in skip_these
        )
        if scalar_with_in_notin:
            ...
            if (
                isinstance(lhs_new, Series)
                and isinstance(rhs_new, DataFrame)
    ...
    def check_pow(self, lhs, arith1, rhs):
        ...
        if (
            ...
            pytest.raises(AssertionError, tm.assert_numpy_array_equal, result, expected)
        else:
            tm.assert_almost_equal(result, expected)
...
class TestOperationsNumExprPandas(object):
    ...
    def test_simple_arith_ops(self):
        ...
        for op in filter(lambda x: x != "//", ops):
            ...
            if op in ("in", "not in"):
                pytest.raises(
                    TypeError, pd.eval, ex, engine=self.engine, parser=self.parser
                )
            else:
    ...
    def test_assignment_fails(self):
        ...
        expr1 = "df = df2"
        pytest.raises(ValueError, self.eval, expr1, local_dict={"df": df, "df2": df2})
