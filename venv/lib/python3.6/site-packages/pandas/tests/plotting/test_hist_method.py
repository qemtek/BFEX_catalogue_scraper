...
class TestDataFrameGroupByPlots(TestPlotBase):
    ...
    def test_grouped_hist_layout(self):
        df = self.hist_df
        pytest.raises(ValueError, df.hist, column="weight", by=df.gender, layout=(1, 1))
        pytest.raises(
            ValueError, df.hist, column="height", by=df.category, layout=(1, 3)
        )
        pytest.raises(
            ValueError, df.hist, column="height", by=df.category, layout=(-1, -1)
        )
