...
class TestHDFStore(Base):
    ...
    def test_api(self):
        ...
        with ensure_clean_path(self.path) as path:
            ...
            # invalid
            df = tm.makeDataFrame()
            pytest.raises(ValueError, df.to_hdf, path, "df", append=True, format="f")
            pytest.raises(
                ValueError, df.to_hdf, path, "df", append=True, format="fixed"
            )

            pytest.raises(TypeError, df.to_hdf, path, "df", append=True, format="foo")
            pytest.raises(TypeError, df.to_hdf, path, "df", append=False, format="bar")

        # File path doesn't exist
        path = ""
        pytest.raises(compat.FileNotFoundError, read_hdf, path, "df")
    ...
    def test_mode(self):
        ...
        def check(mode):
            ...
            with ensure_clean_path(self.path) as path:
                ...
                # conv write
                if mode in ["r", "r+"]:
                    pytest.raises(IOError, df.to_hdf, path, "df", mode=mode)
                ...
                # conv read
                if mode in ["w"]:
                    pytest.raises(ValueError, read_hdf, path, "df", mode=mode)
                else:
    ...
    def test_put(self):
        ...
        with ensure_clean_store(self.path) as store:
            ...
            # not OK, not a table
            pytest.raises(ValueError, store.put, "b", df[10:], append=True)
    ...
    def test_put_compression(self):
        ...
        with ensure_clean_store(self.path) as store:
            ...
            pytest.raises(
                ValueError, store.put, "b", df, format="fixed", complib="zlib"
            )
    ...
    def test_put_compression_blosc(self):
        ...
        with ensure_clean_store(self.path) as store:
            ...
            # can't compress if format='fixed'
            pytest.raises(
                ValueError, store.put, "b", df, format="fixed", complib="blosc"
            )
    ...
    def test_latin_encoding(self):
        ...
        def roundtrip(s, key="data", encoding="latin-1", nan_rep=""):
            with ensure_clean_path(self.path) as store:
                s.to_hdf(store, key, format="table", encoding=encoding, nan_rep=nan_rep)
    ...
    def test_append_with_strings(self):
        ...
        with ensure_clean_store(self.path) as store:
            with catch_warnings(record=True):
                ...
                wp = tm.makePanel()
                wp2 = wp.rename(minor_axis={x: "%s_extra" % x for x in wp.minor_axis})
    ...
    def test_append_with_data_columns(self):
        ...
        with ensure_clean_store(self.path) as store:
            with catch_warnings(record=True):
                ...
                expected = expected[expected["ItemA"] > 0]
                expected = expected[
                    expected.reset_index(level=["major"]).index.isin(["A", "B"])
                ]
                tm.assert_frame_equal(result.to_frame(), expected)
    ...
    def test_append_diff_item_order(self):
        ...
        with catch_warnings(record=True):
            wp = tm.makePanel()
            wp1 = wp.iloc[:, :10, :]
            wp2 = wp.iloc[wp.items.get_indexer(["ItemC", "ItemB", "ItemA"]), 10:, :]

            with ensure_clean_store(self.path) as store:
                store.put("panel", wp1, format="table")
                pytest.raises(ValueError, store.put, "panel", wp2, append=True)
    ...
    def test_column_multiindex(self):
        ...
        with ensure_clean_store(self.path) as store:
            ...
            pytest.raises(
                ValueError, store.put, "df2", df, format="table", data_columns=["A"]
            )
            pytest.raises(
                ValueError, store.put, "df3", df, format="table", data_columns=True
            )
    ...
    def test_pass_spec_to_storer(self):
        ...
        with ensure_clean_store(self.path) as store:
            ...
            pytest.raises(TypeError, store.select, "df", where=[("columns=A")])
    ...
    def test_append_raise(self):
        ...
        with ensure_clean_store(self.path) as store:
            ...
            # series directly
            pytest.raises(TypeError, store.append, "df", Series(np.arange(10)))
    ...
    def test_table_index_incompatible_dtypes(self):
        ...
        with ensure_clean_store(self.path) as store:
            ...
            pytest.raises(
                TypeError, store.put, "frame", df2, format="table", append=True
            )
    ...
    def test_unimplemented_dtypes_table_columns(self):
        ...
        with ensure_clean_store(self.path) as store:
            ...
            for n, f in dtypes:
                df = tm.makeDataFrame()
                df[n] = f
                pytest.raises(TypeError, store.append, "df1_%s" % n, df)
    ...
    def test_remove_startstop(self):
        ...
        with ensure_clean_store(self.path) as store:
            ...
            with catch_warnings(record=True):
                ...
                expected = wp.reindex(
                    major_axis=(
                        wp.major_axis[: 16 // 4].union(wp.major_axis[-16 // 4 :])
                    )
                )
                assert_panel_equal(result, expected)
                ...
                expected = wp.reindex(
                    major_axis=wp.major_axis.difference(
    ...
    def test_remove_crit(self):
        ...
        with ensure_clean_store(self.path) as store:
            ...
            with catch_warnings(record=True):
                ...
                expected = wp.reindex(
                    major_axis=(
                        wp.major_axis.difference(date1).difference(Index([date2]))
                    )
                )
                assert_panel_equal(result, expected)
                ...
                n = store.remove("wp4", where="major_axis>wp.major_axis[-1]")
                result = store.select("wp4")
    ...
    def test_invalid_terms(self):
        ...
        with ensure_clean_store(self.path) as store:
            ...
            with catch_warnings(record=True):
                ...
                # some invalid terms
                pytest.raises(ValueError, store.select, "wp", "minor=['A', 'B']")
                pytest.raises(ValueError, store.select, "wp", ["index=['20121114']"])
                pytest.raises(
                    ValueError, store.select, "wp", ["index=['20121114', '20121114']"]
                )
                pytest.raises(TypeError, Term)

                # more invalid
                pytest.raises(ValueError, store.select, "df", "df.index[3]")
        ...
        with ensure_clean_path(self.path) as path:
            ...
            pytest.raises(ValueError, read_hdf, path, "dfq", where="A>0 or C>0")
    ...
    def test_terms(self):
        ...
        with ensure_clean_store(self.path) as store:
            ...
            with catch_warnings(record=True):
                ...
                # with deprecation
                result = store.select(
                    "wp", where=("major_axis<'20000108' " "and minor_axis=['A', 'B']")
                )
                expected = wp.truncate(after="20000108").reindex(minor=["A", "B"])
            ...
            with catch_warnings(record=True):
                ...
                with pytest.raises(
                    TypeError, match="Only named functions are supported"
                ):
                    store.select("wp", 'major_axis == (lambda x: x)("20130101")')
    ...
    def test_term_compat(self):
        with ensure_clean_store(self.path) as store:
            ...
            with catch_warnings(record=True):
                ...
                result = store.select(
                    "wp", where=("major_axis>20000102 " "and minor_axis=['A', 'B']")
                )
    ...
    def test_select_iterator(self):
        ...
        with ensure_clean_path(self.path) as path:
            ...
            pytest.raises(TypeError, read_hdf, path, "df_non_table", chunksize=100)
            pytest.raises(TypeError, read_hdf, path, "df_non_table", iterator=True)
    ...
    def test_retain_index_attributes(self):
        ...
        df = DataFrame(
            dict(A=Series(lrange(3), index=date_range("2000-1-1", periods=3, freq="H")))
        )
        ...
        with ensure_clean_store(self.path) as store:
            ...
            with catch_warnings(record=True):
                df2 = DataFrame(
                    dict(
                        A=Series(
                            lrange(3), index=date_range("2002-1-1", periods=3, freq="D")
                        )
                    )
                )
        ...
        with ensure_clean_path(self.path) as path:
            ...
            with catch_warnings(record=True):
                ...
                df = DataFrame(
                    dict(
                        A=Series(
                            lrange(3), index=date_range("2000-1-1", periods=3, freq="H")
                        )
                    )
                )
    ...
    def test_frame_select_complex(self):
        ...
        with ensure_clean_store(self.path) as store:
            ...
            pytest.raises(NotImplementedError, store.select, "df", '~(string="bar")')
    ...
    def test_invalid_filtering(self):
        ...
        with ensure_clean_store(self.path) as store:
            ...
            # not implemented
            pytest.raises(
                NotImplementedError, store.select, "df", "columns=['A'] | columns=['B']"
            )
    ...
    def test_read_column(self):
        ...
        with ensure_clean_store(self.path) as store:
            ...
            pytest.raises(ValueError, store.select_column, "df", "values_block_0")
    ...
    def test_coordinates(self):
        ...
        with ensure_clean_store(self.path) as store:
            ...
            pytest.raises(ValueError, store.select, "df", where=np.arange(len(df) + 1))
            pytest.raises(
                ValueError, store.select, "df", where=np.arange(len(df)), start=5
            )
            pytest.raises(
                ValueError,
                store.select,
                "df",
                where=np.arange(len(df)),
                start=5,
                stop=10,
            )
    ...
    def test_append_to_multiple(self):
        ...
        with ensure_clean_store(self.path) as store:
            ...
            pytest.raises(ValueError, store.append_to_multiple, "df1", df, "df1")
    ...
    def test_legacy_table_read(self, datapath):
        # legacy table types
        with ensure_clean_store(
            datapath("io", "data", "legacy_hdf", "legacy_table.h5"), mode="r"
        ) as store:
            ...
            with catch_warnings():
                ...
                # old version warning
                pytest.raises(Exception, store.select, "wp1", "minor_axis=B")
    ...
    def test_duplicate_column_name(self):
        ...
        with ensure_clean_path(self.path) as path:
            pytest.raises(ValueError, df.to_hdf, path, "df", format="fixed")
...
class TestHDFComplexValues(Base):
    ...
    def test_complex_indexing_error(self):
        ...
        with ensure_clean_store(self.path) as store:
            pytest.raises(TypeError, store.append, "df", df, data_columns=["C"])
