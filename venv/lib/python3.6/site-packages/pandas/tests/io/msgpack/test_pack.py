...
from pandas.io.msgpack import Packer
from pandas.io.msgpack import unpackb
from pandas.io.msgpack import Unpacker
...
class TestPack(object):
    ...
    def testStrictUnicodePack(self):
        msg = (
            r"'ascii' codec can't encode character u*'\\xed' in position 3:"
            r" ordinal not in range\(128\)"
        )
        with pytest.raises(UnicodeEncodeError, match=msg):
            packb(compat.u("abc\xeddef"), encoding="ascii", unicode_errors="strict")

    def testIgnoreErrorsPack(self):
        re = unpackb(
            packb(compat.u("abcФФФdef"), encoding="ascii", unicode_errors="ignore"),
            encoding="utf-8",
            use_list=1,
        )
