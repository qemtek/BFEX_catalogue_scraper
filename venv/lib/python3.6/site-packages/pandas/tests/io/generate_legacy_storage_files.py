...
def create_data():
    ...
    series = dict(
        ...
        mi=Series(
            ...
            index=MultiIndex.from_tuples(
                tuple(zip(*[[1, 1, 2, 2, 2], [3, 4, 3, 4, 5]])), names=[u"one", u"two"]
            ),
        ),
        dup=Series(
            np.arange(5).astype(np.float64), index=[u"A", u"B", u"C", u"D", u"A"]
        ),
    ...
    frame = dict(
        float=DataFrame({u"A": series[u"float"], u"B": series[u"float"] + 1}),
        int=DataFrame({u"A": series[u"int"], u"B": series[u"int"] + 1}),
        mixed=DataFrame({k: data[k] for k in [u"A", u"B", u"C", u"D"]}),
        mi=DataFrame(
            {
        ...
        dup=DataFrame(
            np.arange(15).reshape(5, 3).astype(np.float64), columns=[u"A", u"B", u"A"]
        ),
    ...
    with catch_warnings(record=True):
        ...
        mixed_dup_panel = Panel({u"ItemA": frame[u"float"], u"ItemB": frame[u"int"]})
        mixed_dup_panel.items = [u"ItemA", u"ItemA"]
        panel = dict(
            float=Panel({u"ItemA": frame[u"float"], u"ItemB": frame[u"float"] + 1}),
            dup=Panel(
    ...
    if _loose_version < LooseVersion("0.19.2"):
        ...
        timestamp["both"] = Timestamp("2011-01-01", tz="Asia/Tokyo", offset="M")
    else:
