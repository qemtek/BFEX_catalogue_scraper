...
from pandas.io.excel import _XlwtWriter
...
class ReadingTestsBase(SharedItems):
        ...
        with tm.assert_produces_warning(FutureWarning, check_stacklevel=False):
            with ignore_xlrd_time_clock_warning():
                df3 = self.get_exceldf(
                    "test1", ext, "Sheet2", skiprows=[1], index_col=0, parse_cols=3
                )
    ...
    def test_usecols_list(self, ext):
        ...
        df2 = self.get_exceldf(
            "test1", ext, "Sheet2", skiprows=[1], index_col=0, usecols=[0, 2, 3]
        )
    ...
    def test_usecols_str(self, ext):
        ...
        df2 = self.get_exceldf("test1", ext, "Sheet1", index_col=0, usecols="A:D")
        df3 = self.get_exceldf(
            "test1", ext, "Sheet2", skiprows=[1], index_col=0, usecols="A:D"
        )

        with tm.assert_produces_warning(FutureWarning):
            with ignore_xlrd_time_clock_warning():
                df4 = self.get_exceldf(
                    "test1", ext, "Sheet2", skiprows=[1], index_col=0, parse_cols="A:D"
                )
        ...
        df2 = self.get_exceldf("test1", ext, "Sheet1", index_col=0, usecols="A,C,D")
        df3 = self.get_exceldf(
    ...
    def test_usecols_diff_positional_int_columns_order(self, ext, usecols):
        ...
        result = self.get_exceldf("test1", ext, "Sheet1", index_col=0, usecols=usecols)
    ...
    def test_usecols_excel_range_str(self, ext):
        ...
        result = self.get_exceldf("test1", ext, "Sheet1", index_col=0, usecols="A,D:E")
        tm.assert_frame_equal(result, expected, check_names=False)
    ...
    def test_index_col_label_error(self, ext):
        ...
        with pytest.raises(TypeError, match=msg):
            self.get_exceldf(
                "test1", ext, "Sheet1", index_col=["A"], usecols=["A", "C"]
            )

    def test_index_col_empty(self, ext):
        # see gh-9208
        result = self.get_exceldf("test1", ext, "Sheet3", index_col=["A", "B", "C"])
        expected = DataFrame(
            columns=["D", "E", "F"],
    ...
    def test_index_col_with_unnamed(self, ext, index_col):
        # see gh-18792
        result = self.get_exceldf("test1", ext, "Sheet4", index_col=index_col)
        expected = DataFrame(
    ...
    def test_excel_passes_na(self, ext):
        ...
        parsed = read_excel(excel, "Sheet1", keep_default_na=False, na_values=["apple"])
        expected = DataFrame(
            [["NA"], [1], ["NA"], [np.nan], ["rabbit"]], columns=["Test"]
        )
        tm.assert_frame_equal(parsed, expected)

        parsed = read_excel(excel, "Sheet1", keep_default_na=True, na_values=["apple"])
        expected = DataFrame(
    ...
    def test_excel_table(self, ext):
        ...
        df3 = self.get_exceldf("test1", ext, "Sheet1", index_col=0, skipfooter=1)
        tm.assert_frame_equal(df3, df1.iloc[:-1])
    ...
    def test_reader_special_dtypes(self, ext):
        ...
        actual = self.get_exceldf(basename, ext, "Sheet1", converters={"StrCol": str})
        tm.assert_frame_equal(actual, expected)

        no_convert_float = float_expected.copy()
        no_convert_float["StrCol"] = no_convert_float["StrCol"].apply(str)
        actual = self.get_exceldf(
            basename, ext, "Sheet1", convert_float=False, converters={"StrCol": str}
        )
    ...
    def test_reader_dtype(self, ext):
        ...
        actual = self.get_exceldf(
            basename, ext, dtype={"a": "float64", "b": "float32", "c": str}
        )
    ...
    def test_set_column_names_in_parameter(self, ext):
        ...
        with ensure_clean(ext) as pth:
            ...
            with ExcelFile(pth) as reader:
                xlsdf_no_head = read_excel(
                    reader, "Data_no_head", header=None, names=["A", "B"]
                )
                xlsdf_with_head = read_excel(
    ...
    def test_sheet_name_and_sheetname(self, ext):
        ...
        df1 = self.get_exceldf(filename, ext, sheet_name=sheet_name, index_col=0)  # doc
        with tm.assert_produces_warning(FutureWarning, check_stacklevel=False):
            with ignore_xlrd_time_clock_warning():
                df2 = self.get_exceldf(
                    filename, ext, index_col=0, sheetname=sheet_name
                )  # backward compat

        excel = self.get_excelfile(filename, ext)
        df1_parse = excel.parse(sheet_name=sheet_name, index_col=0)  # doc
        with tm.assert_produces_warning(FutureWarning, check_stacklevel=False):
            df2_parse = excel.parse(
                index_col=0, sheetname=sheet_name
            )  # backward compat
    ...
    def test_sheet_name_both_raises(self, ext):
        ...
        excel = self.get_excelfile("test1", ext)
        with pytest.raises(TypeError, match="Cannot specify both"):
            excel.parse(sheetname="Sheet1", sheet_name="Sheet1")
    ...
    def test_read_from_s3_url(self, ext, s3_resource):
        ...
        with open(file_name, "rb") as f:
            s3_resource.Bucket("pandas-test").put_object(Key="test1" + ext, Body=f)
    ...
    def test_read_excel_bool_header_arg(self, ext):
        ...
        for arg in [True, False]:
            with pytest.raises(TypeError):
                pd.read_excel(os.path.join(self.dirpath, "test1" + ext), header=arg)

    def test_read_excel_chunksize(self, ext):
        # GH 8011
        with pytest.raises(NotImplementedError):
            pd.read_excel(os.path.join(self.dirpath, "test1" + ext), chunksize=100)
    ...
    def test_read_excel_skiprows_list(self, ext):
        ...
        actual = pd.read_excel(
            os.path.join(self.dirpath, "testskiprows" + ext),
            "skiprows_list",
            skiprows=[0, 2],
        )
        expected = DataFrame(
    ...
    def test_read_excel_nrows(self, ext):
        ...
        num_rows_to_pull = 5
        actual = pd.read_excel(
            os.path.join(self.dirpath, "test1" + ext), nrows=num_rows_to_pull
        )
        expected = pd.read_excel(os.path.join(self.dirpath, "test1" + ext))
    ...
    def test_read_excel_nrows_non_integer_parameter(self, ext):
        ...
        with pytest.raises(ValueError, match=msg):
            pd.read_excel(os.path.join(self.dirpath, "test1" + ext), nrows="5")
...
class TestXlrdReader(ReadingTestsBase):
    ...
    def test_read_xlrd_book(self, ext):
        ...
        with ensure_clean(ext) as pth:
            ...
            result = read_excel(book, sheet_name=sheet_name, engine=engine, index_col=0)
            tm.assert_frame_equal(df, result)
...
class TestExcelWriter(_WriterBase):
    ...
    def test_roundtrip(self, merge_cells, engine, ext):
        ...
        self.frame.to_excel(self.path, "test1", na_rep="88")
        recons = read_excel(self.path, "test1", index_col=0, na_values=[88, 88.0])
        tm.assert_frame_equal(self.frame, recons)
    ...
    def test_int_types(self, merge_cells, engine, ext, np_type):
        ...
        frame = DataFrame(np.random.randint(-10, 10, size=(10, 2)), dtype=np_type)
        ...
        recons = read_excel(self.path, "test1", convert_float=False, index_col=0)
    ...
    def test_roundtrip_indexlabels(self, merge_cells, engine, ext):
        ...
        reader = ExcelFile(self.path)
        recons = read_excel(reader, "test1", index_col=0).astype(np.int64)
        ...
        frame = DataFrame(np.random.randn(10, 2)) >= 0
        frame.to_excel(self.path, "test1", index_label="test", merge_cells=merge_cells)
        reader = ExcelFile(self.path)
        recons = read_excel(reader, "test1", index_col=0).astype(np.int64)
        frame.index.names = ["test"]
    ...
    def test_excel_roundtrip_indexname(self, merge_cells, engine, ext):
        ...
        result = read_excel(xf, xf.sheet_names[0], index_col=0)
    ...
    def test_to_excel_interval_no_labels(self, *_):
        ...
        frame = DataFrame(np.random.randint(-10, 10, size=(20, 1)), dtype=np.int64)
        expected = frame.copy()
    ...
    def test_to_excel_interval_labels(self, *_):
        ...
        expected = frame.copy()
        intervals = pd.cut(
            frame[0], 10, labels=["A", "B", "C", "D", "E", "F", "G", "H", "I", "J"]
        )
        frame["new"] = intervals
    ...
    def test_to_excel_multiindex_cols(self, merge_cells, engine, ext):
        ...
        frame.to_excel(self.path, "test1", merge_cells=merge_cells)
        reader = ExcelFile(self.path)
        df = read_excel(reader, "test1", header=header, index_col=[0, 1])
        if not merge_cells:
    ...
    def test_to_excel_output_encoding(self, merge_cells, engine, ext):
        ...
        with ensure_clean("__tmp_to_excel_float_format__." + ext) as filename:
            ...
            result = read_excel(filename, "TestSheet", encoding="utf8", index_col=0)
            tm.assert_frame_equal(result, df)
    ...
    def test_duplicated_columns(self, *_):
        ...
        result = read_excel(self.path, "test1", index_col=0, mangle_dupe_cols=True)
        tm.assert_frame_equal(result, expected)
...
@td.skip_if_no("openpyxl")
@pytest.mark.parametrize("merge_cells,ext,engine", [(None, ".xlsx", "openpyxl")])
...
@td.skip_if_no("xlwt")
@pytest.mark.parametrize("merge_cells,ext,engine", [(None, ".xls", "xlwt")])
class TestXlwtTests(_WriterBase):
    ...
    def test_excel_multiindex_columns_and_index_true(self, merge_cells, ext, engine):
        cols = MultiIndex.from_tuples(
            [("site", ""), ("2014", "height"), ("2014", "weight")]
        )
...
@td.skip_if_no("xlsxwriter")
@pytest.mark.parametrize("merge_cells,ext,engine", [(None, ".xlsx", "xlsxwriter")])
