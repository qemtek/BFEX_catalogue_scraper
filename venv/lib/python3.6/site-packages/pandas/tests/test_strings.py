...
class TestStringMethods(object):
    ...
    def test_api_per_dtype(self, box, dtype, any_skipna_inferred_dtype):
        ...
        if box == Index and inferred_dtype in ["empty", "bytes"]:
            pytest.xfail(reason="Raising too restrictively; " "solved by GH 23167")
        if (
            box == Index
            and dtype == object
            ...
            pytest.xfail(
                reason="Inferring incorrectly because of NaNs; " "solved by GH 23167"
            )
        if (
            box == Series
            and (
                dtype == object
                and inferred_dtype
                not in ["string", "unicode", "empty", "bytes", "mixed", "mixed-integer"]
            )
            or (
                dtype == "category" and inferred_dtype in ["decimal", "boolean", "time"]
            )
        ):
    ...
    def test_api_per_method(
        ...
        # TODO: get rid of these xfails
        if method_name not in ["encode", "decode", "len"] and inferred_dtype == "bytes":
            pytest.xfail(
                reason='Not raising for "bytes", see GH 23011;'
                "Also: malformed method names, see GH 23551; "
                "solved by GH 23167"
            )
        if method_name == "cat" and inferred_dtype in ["mixed", "mixed-integer"]:
            pytest.xfail(
                reason="Bad error message; should raise better; " "solved by GH 23167"
            )
        if box == Index and inferred_dtype in ["empty", "bytes"]:
            pytest.xfail(reason="Raising too restrictively; " "solved by GH 23167")
        if (
    ...
    def test_contains(self):
        ...
        # unicode
        values = np.array([u"foo", NA, u"fooommm__foo", u"mmm_"], dtype=np.object_)
        pat = "mmm[_]+"
    ...
    def test_startswith(self):
        ...
        # unicode
        values = Series(
            [u("om"), NA, u("foo_nom"), u("nom"), u("bar_foo"), NA, u("foo")]
        )
    ...
    def test_partition_series(self):
        ...
        result = values.str.partition("_", expand=False)
        exp = Series(
            [(u"a", u"_", u"b_c"), (u"c", u"_", u"d_e"), NA, (u"f", u"_", u"g_h")]
        )
        tm.assert_series_equal(result, exp)

        result = values.str.rpartition("_", expand=False)
        exp = Series(
            [(u"a_b", u"_", u"c"), (u"c_d", u"_", u"e"), NA, (u"f_g", u"_", u"h")]
        )
        tm.assert_series_equal(result, exp)
    ...
    def test_normalize(self):
        ...
        expected = Series(
            [u"ABC", u"ＡＢＣ", u"１２３", np.nan, u"ｱｲｴ"], index=["a", "b", "c", "d", "e"]
        )
