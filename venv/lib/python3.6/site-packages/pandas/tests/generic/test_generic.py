    ...
    def test_head_tail(self):
        ...
        for index in [
            ...
            try:
                ...
                pytest.skip("not implemented on {0}".format(o.__class__.__name__))
...
class TestNDFrame(object):
    ...
    def test_sample(sel):
        ...
        with catch_warnings(record=True):
            ...
            panel = Panel(items=[0, 1, 2], major_axis=[2, 3, 4], minor_axis=[3, 4, 5])
            with pytest.raises(ValueError):
        ...
        with catch_warnings(record=True):
            simplefilter("ignore", FutureWarning)
            p = Panel(items=["a", "b", "c"], major_axis=[2, 4, 6], minor_axis=[1, 3, 5])
            assert_panel_equal(
                p.sample(n=3, random_state=42), p.sample(n=3, axis=1, random_state=42)
            )
            assert_frame_equal(
    ...
    def test_transpose(self):
        msg = r"transpose\(\) got multiple values for " r"keyword argument 'axes'"
        for s in [tm.makeFloatSeries(), tm.makeStringSeries(), tm.makeObjectSeries()]:
        ...
        with catch_warnings(record=True):
            simplefilter("ignore", FutureWarning)
            for p in [tm.makePanel()]:
                tm.assert_panel_equal(p.transpose(2, 0, 1).transpose(1, 2, 0), p)
    ...
    def test_numpy_transpose(self):
        ...
        with catch_warnings(record=True):
            ...
            tm.assert_panel_equal(
                np.transpose(np.transpose(p, axes=(2, 0, 1)), axes=(1, 2, 0)), p
            )
    ...
    def test_take(self):
        ...
        with catch_warnings(record=True):
            ...
            for p in [tm.makePanel()]:
                out = p.take(indices)
                expected = Panel(
                    data=p.values.take(indices, axis=0),
