...
class TestFrequencyCode(object):
    ...
    def test_get_to_timestamp_base(self):
        ...
        assert tsb(get_freq_code("D")[0]) == get_freq_code("D")[0]
        assert tsb(get_freq_code("W")[0]) == get_freq_code("D")[0]
        assert tsb(get_freq_code("M")[0]) == get_freq_code("D")[0]

        assert tsb(get_freq_code("S")[0]) == get_freq_code("S")[0]
        assert tsb(get_freq_code("T")[0]) == get_freq_code("S")[0]
        assert tsb(get_freq_code("H")[0]) == get_freq_code("S")[0]
    ...
    def test_resolution_bumping(self):
        ...
        assert Reso.get_stride_from_decimal(0.342931, "H") == (1234551600, "U")
    ...
    def test_get_freq_code(self):
        ...
        # Monday is weekday=0
        assert get_freq_code(offsets.Week(weekday=1)) == (get_freq("W-TUE"), 1)
        assert get_freq_code(offsets.Week(3, weekday=0)) == (get_freq("W-MON"), 3)
        assert get_freq_code(offsets.Week(-2, weekday=4)) == (get_freq("W-FRI"), -2)

    def test_frequency_misc(self):
        assert resolution.get_freq_group("T") == FreqGroup.FR_MIN
...
class TestFrequencyInference(object):
    ...
    def test_string_datetimelike_compat(self):
        ...
        result = frequencies.infer_freq(
            Index(["2004-01", "2004-02", "2004-03", "2004-04"])
        )
        assert result == expected
