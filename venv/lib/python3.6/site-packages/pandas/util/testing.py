...
from pandas.compat import filter
from pandas.compat import httplib
...
from pandas.core.dtypes.common import is_datetimelike_v_object
...
RANDU_CHARS = np.array(
    list(u("").join(map(unichr, lrange(1488, 1488 + 26))) + string.digits),
    dtype=(np.unicode_, 1),
)
...
def set_defaultencoding(encoding):
    ...
    if not PY2:
        raise ValueError(
            "set_defaultencoding context is only available " "in Python 2."
        )
...
def assert_sp_array_equal(
    ...
    if check_dtype:
        assert_attr_equal("dtype", left, right)
    assert_numpy_array_equal(left.values, right.values, check_dtype=check_dtype)
...
def assert_produces_warning(
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert not extra_warnings, ("Caused unexpected warning(s): {extra!r}.").format(
