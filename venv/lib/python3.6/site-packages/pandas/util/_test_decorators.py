...
skip_if_np_lt_115 = pytest.mark.skipif(
    _np_version_under1p15, reason="NumPy 1.15 or greater required"
)
skip_if_mpl = pytest.mark.skipif(not _skip_if_no_mpl(), reason="matplotlib is present")
xfail_if_mpl_2_2 = pytest.mark.xfail(
    _skip_if_mpl_2_2(), reason="matplotlib 2.2", strict=False
)
...
skip_if_windows_python_3 = pytest.mark.skipif(
    is_platform_windows() and PY3, reason=("not used on python3/" "win32")
)
...
skip_if_no_lzma = pytest.mark.skipif(
    _skip_if_no_lzma(), reason="need backports.lzma to run"
)
