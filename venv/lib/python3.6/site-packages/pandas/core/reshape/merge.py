...
class _AsOfMerge(_OrderedMerge):
    ...
    def _get_merge_keys(self):
        ...
        # note this function has side effects
        (left_join_keys, right_join_keys, join_names) = super(
            _AsOfMerge, self
        )._get_merge_keys()
            ...
            if not is_dtype_equal(lk.dtype, rk.dtype):
                raise MergeError(
                    ...
                    "{rkdtype}, must be the same type".format(
                        i=i, lkdtype=lk.dtype, rkdtype=rk.dtype
                    )
                )
...
def _should_fill(lname, rname):
    if not isinstance(lname, compat.string_types) or not isinstance(
        rname, compat.string_types
    ):
        return True
    return lname == rname
