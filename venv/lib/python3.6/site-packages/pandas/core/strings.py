...
class StringMethods(NoNewAttributesMixin):
    ...
    def _validate(data):
        ...
        if isinstance(data, ABCSeries) and not (
            (
                is_categorical_dtype(data.dtype)
                and is_object_dtype(data.values.categories)
            )
            or (is_object_dtype(data.dtype))
        ):
    ...
    def cat(self, others=None, sep=None, na_rep=None, join=None):
        ...
        if na_rep is None and union_mask.any():
            ...
            result[not_masked] = cat_core([x[not_masked] for x in all_cols], sep)
