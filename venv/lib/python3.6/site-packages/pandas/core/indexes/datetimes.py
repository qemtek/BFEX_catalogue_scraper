...
from pandas._libs import tslib as libts
...
from pandas.core.dtypes.common import is_string_like
from pandas.core.dtypes.dtypes import DatetimeTZDtype
from pandas.core.dtypes.missing import isna
from pandas.core.indexes.base import Index
from pandas.core.indexes.datetimelike import DatetimeIndexOpsMixin
from pandas.core.indexes.datetimelike import DatetimelikeDelegateMixin
from pandas.core.indexes.datetimelike import ea_passthrough
...
class DatetimeIndex(DatetimeIndexOpsMixin, Int64Index, DatetimeDelegateMixin):
    ...
    def to_series(self, keep_tz=None, index=None, name=None):
        ...
        if keep_tz is None and self.tz is not None:
            warnings.warn(
                "The default of the 'keep_tz' keyword will change "
    ...
    def _parsed_string_to_bounds(self, reso, parsed):
        ...
        if reso == "year":
            return (
                Timestamp(datetime(parsed.year, 1, 1), tz=self.tz),
                Timestamp(
                    datetime(parsed.year, 12, 31, 23, 59, 59, 999999), tz=self.tz
                ),
            )
        elif reso == "month":
            d = ccalendar.get_days_in_month(parsed.year, parsed.month)
            return (
                Timestamp(datetime(parsed.year, parsed.month, 1), tz=self.tz),
                Timestamp(
                    datetime(parsed.year, parsed.month, d, 23, 59, 59, 999999),
                    tz=self.tz,
                ),
            )
        elif reso == "quarter":
            ...
            st = datetime(parsed.year, parsed.month, parsed.day, hour=parsed.hour)
            return (
                Timestamp(st, tz=self.tz),
                Timestamp(Timestamp(st + offsets.Hour(), tz=self.tz).value - 1),
            )
        elif reso == "minute":
            st = datetime(
                parsed.year,
            ...
            return (
                Timestamp(st, tz=self.tz),
                Timestamp(Timestamp(st + offsets.Minute(), tz=self.tz).value - 1),
            )
        elif reso == "second":
            st = datetime(
                parsed.year,
            ...
            return (
                Timestamp(st, tz=self.tz),
                Timestamp(Timestamp(st + offsets.Second(), tz=self.tz).value - 1),
            )
    ...
    def indexer_at_time(self, time, asof=False):
        ...
        if time.tzinfo:
            # TODO
            raise NotImplementedError(
                "argument 'time' with timezone info is " "not supported"
            )
