...
class SeriesGroupBy(GroupBy):
    ...
    for _def_str in base.whitelist_method_generator(GroupBy, Series, _apply_whitelist):
        exec(_def_str)
    ...
    def _wrap_applied_output(self, keys, values, not_indexed_same=False):
        ...
        if isinstance(values[0], (Series, dict)):
            ...
            # GH #6265
            return Series(values, index=_get_index(), name=self._selection_name)
...
class DataFrameGroupBy(NDFrameGroupBy):
    ...
    def _gotitem(self, key, ndim, subset=None):
        ...
        if ndim == 2:
            ...
            if subset is None:
                subset = self.obj[key]
            return SeriesGroupBy(subset, selection=key, grouper=self.grouper)
    ...
    def _reindex_output(self, result):
        ...
        result = result.set_index(self.grouper.result_index).reindex(index, copy=False)
...
class PanelGroupBy(NDFrameGroupBy):
    ...
    def _aggregate_item_by_item(self, func, *args, **kwargs):
        ...
        if self.axis > 0:
            for item in obj:
                try:
                    itemg = DataFrameGroupBy(
                        obj[item], axis=self.axis - 1, grouper=self.grouper
                    )
