...
from pandas.core.dtypes.common import ensure_int64_or_float64
...
from pandas.core.dtypes.missing import _maybe_fill
...
from pandas.core.sorting import get_flattened_iterator
    ...
    def _get_cython_function(self, kind, how, values, is_numeric):
        ...
        def get_func(fname):
            ...
            for dt in [dtype_str, "object"]:
                f = getattr(
                    libgroupby,
                    "{fname}_{dtype_str}".format(fname=fname, dtype_str=dtype_str),
                    None,
                )
                if f is not None:
                    return f
        ...
        **kwargs
        ...
        if values.ndim > 3:
            ...
            for i, chunk in enumerate(values.transpose(2, 0, 1)):
                ...
                transform_func(
                    result[:, :, i], values, comp_ids, is_datetimelike, **kwargs
                )
        else:
...
class FrameSplitter(DataSplitter):
    def fast_apply(self, f, names):
        ...
        sdata = self._get_sorted_data()
        results, mutated = reduction.apply_frame_axis0(sdata, f, names, starts, ends)
