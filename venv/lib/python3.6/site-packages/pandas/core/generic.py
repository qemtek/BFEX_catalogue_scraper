...
from pandas.compat import isidentifier
...
class NDFrame(PandasObject, SelectionMixin):
    ...
    _deprecations = frozenset(["as_blocks", "blocks", "convert_objects", "is_copy"])
    _metadata = []
    _is_copy = None
    ...
    def _get_axis_number(cls, axis):
        ...
        if is_integer(axis):
            ...
            try:
                ...
                pass
        raise ValueError(
            "No axis named {0} for object type {1}".format(axis, type(cls))
        )
    ...
    def _is_label_or_level_reference(self, key, axis=0):
        ...
        if self.ndim > 2:
            raise NotImplementedError(
                "_is_label_or_level_reference is not implemented for {type}".format(
                    type=type(self)
                )
            )
    ...
    def _check_label_or_level_ambiguity(self, key, axis=0):
        ...
        if self.ndim > 2:
            raise NotImplementedError(
                "_check_label_or_level_ambiguity is not implemented for {type}".format(
                    type=type(self)
                )
            )
    ...
    def _get_label_or_level_values(self, key, axis=0):
        ...
        if self.ndim > 2:
            raise NotImplementedError(
                "_get_label_or_level_values is not implemented for {type}".format(
                    type=type(self)
                )
            )
    ...
    def _drop_labels_or_levels(self, keys, axis=0):
        ...
        if self.ndim > 2:
            raise NotImplementedError(
                "_drop_labels_or_levels is not implemented for {type}".format(
                    type=type(self)
                )
            )
    ...
    **kwargs
    ...
    def fillna(
        ...
        if value is None:
            ...
            # 3d
                ...
                # fill in 2d chunks
                result = {
                    col: s.fillna(method=method, value=value)
        ...
        if value is None:
            ...
            if is_dict_like(to_replace):
                ...
                # {'A': NA} -> 0
                    ...
                    keys = [
                        (k, src) for k, src in compat.iteritems(to_replace) if k in self
                    ]
                    keys_len = len(keys) - 1
    ...
    def asof(self, where, subset=None):
        ...
        if is_series:
            ...
            raise NotImplementedError(
                "asof is not implemented " "for {type}".format(type=type(self))
            )
        else:
            if subset is None:
                subset = self.columns
    ...
    @Appender(_shared_docs["valid_index"] % {"position": "first", "klass": "NDFrame"})
    def first_valid_index(self):
        return self._find_valid_index("first")

    @Appender(_shared_docs["valid_index"] % {"position": "last", "klass": "NDFrame"})
...
def _make_stat_function_ddof(cls, name, name1, name2, axis_descr, desc, f):
    @Substitution(
        outname=name, desc=desc, name1=name1, name2=name2, axis_descr=axis_descr
    )
