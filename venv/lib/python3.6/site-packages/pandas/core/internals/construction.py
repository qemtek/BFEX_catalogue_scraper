...
from pandas.core.dtypes.cast import maybe_castable
...
def init_ndarray(values, index, columns, dtype=None, copy=False):
    ...
    elif is_datetime64tz_dtype(values) or is_extension_array_dtype(values):
        # GH#19157
        if columns is None:
            columns = [0]
        return arrays_to_mgr([values], columns, index, columns, dtype=dtype)
...
def init_dict(data, index, columns, dtype=None):
    ...
    if columns is not None:
        ...
        for key in data:
            if isinstance(data[key], ABCDatetimeIndex) and data[key].tz is not None:
