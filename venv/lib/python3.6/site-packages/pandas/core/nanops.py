...
from pandas.core.dtypes.cast import _int64_max
...
try:
    ...
    if not _BOTTLENECK_INSTALLED:
        warnings.warn(
            ...
            "version is {min_ver}\n".format(ver=ver, min_ver=_MIN_BOTTLENECK_VERSION),
            UserWarning,
        )
...
def nanany(values, axis=None, skipna=True, mask=None):
    ...
    values, mask, dtype, _, _ = _get_values(
        values, skipna, False, copy=skipna, mask=mask
    )
...
def nanall(values, axis=None, skipna=True, mask=None):
    ...
    """
    values, mask, dtype, _, _ = _get_values(
        values, skipna, True, copy=skipna, mask=mask
    )
