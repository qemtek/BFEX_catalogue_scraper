...
from pandas.compat import ResourceWarning
...
class StataReader(StataParser, BaseIterator):
    ...
    def _get_varlist(self):
        ...
        return [
            self._null_terminate(self.path_or_buf.read(b)) for i in range(self.nvar)
        ]
    ...
    def _read_value_labels(self):
        ...
        while True:
            ...
            for i in range(n):
                ...
                if self.format_version <= 117:
                    self.value_label_dict[labname][val[i]] = self._null_terminate(
                        txt[off[i] : end]
                    )
                else:
    ...
    def read(
        ...
        if convert_dates:
            cols = np.where(
                lmap(
                    lambda x: any(x.startswith(fmt) for fmt in _date_formats),
                    self.fmtlist,
                )
            )[0]
            for i in cols:
        ...
        for col, label in zip(data, lbllist):
            if label in value_labels:
                ...
                try:
                    ...
                    repeats = "\n" + "-" * 80 + "\n".join(repeats)
                    raise ValueError(
                        "Value labels for column {col} are not "
                        "unique. The repeated labels are:\n"
                        "{repeats}".format(col=col, repeats=repeats)
                    )
...
class StataWriter117(StataWriter):
    ...
    def __init__(
        ...
        super(StataWriter117, self).__init__(
            fname,
            data,
            convert_dates,
            write_index,
    ...
    def _tag(val, tag):
        ...
        return (
            _bytes("<" + tag + ">", "utf-8") + val + _bytes("</" + tag + ">", "utf-8")
        )
