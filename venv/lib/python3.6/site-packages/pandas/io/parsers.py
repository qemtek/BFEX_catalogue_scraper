...
from pandas.io.common import UnicodeReader
from pandas.io.common import UTF8Recoder
...
class TextFileReader(BaseIterator):
    ...
    def _clean_options(self, options, engine):
        ...
        if quotechar is not None and isinstance(
            quotechar, (str, compat.text_type, bytes)
        ):
            if (
                len(quotechar) == 1
    ...
    def _get_complex_date_index(self, data, col_names):
        def _get_name(icol):
            ...
            if col_names is None:
                raise ValueError(
                    ("Must supply column order to use %s as " "index") % str(icol)
                )
    ...
    def _cast_types(self, values, cast_type, column):
        ...
        # use the EA's implementation of casting
            ...
            try:
                ...
                raise ValueError(
                    "Unable to convert column %s to " "type %s" % (column, cast_type)
                )
        return values
...
class PythonParser(ParserBase):
    def __init__(self, f, **kwds):
        ...
        if self.thousands is None:
            ...
            self.nonnum = re.compile("[^-^0-9^%s^%s]+" % (self.thousands, self.decimal))
    ...
    def _check_comments(self, lines):
        ...
        for l in lines:
            rl = []
            for x in l:
                if not isinstance(x, compat.string_types) or self.comment not in x:
                    rl.append(x)
                else:
    ...
    def _remove_empty_lines(self, lines):
        ...
        for l in lines:
            ...
            if (
                len(l) > 1
                or len(l) == 1
                and (not isinstance(l[0], compat.string_types) or l[0].strip())
            ):
                ret.append(l)
        return ret
...
def _make_date_converter(date_parser=None, dayfirst=False, infer_datetime_format=False):
...
def _concat_date_cols(date_cols):
    ...
    rs = np.array(
        [" ".join(compat.text_type(y) for y in x) for x in zip(*date_cols)],
        dtype=object,
    )
    return rs
