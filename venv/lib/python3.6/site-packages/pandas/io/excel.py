    ...
    for ext in klass.supported_extensions:
        ...
        if ext not in _writer_extensions:
            config.register_option(
                "io.excel.{ext}.writer".format(ext=ext), engine_name, validator=str
            )
    ...
    def __init__(self, filepath_or_buffer):
        ...
        try:
            ...
            if xlrd.__VERSION__ < LooseVersion("1.0.0"):
                raise ImportError(err_msg + ". Current version " + xlrd.__VERSION__)
    ...
    def parse(
        ...
        **kwds
        ...
        if is_integer(sheet_name) and sheet_name == 0 and "sheetname" in kwds:
            ...
            raise TypeError(
                "Cannot specify both `sheet_name` "
...
class ExcelWriter(object):
    ...
    def __new__(cls, path, engine=None, **kwargs):
        ...
        if issubclass(cls, ExcelWriter):
            if engine is None or (
                isinstance(engine, string_types) and engine == "auto"
            ):
    ...
    def check_extension(cls, ext):
        ...
        if not any(ext in extension for extension in cls.supported_extensions):
            msg = u("Invalid extension for engine '{engine}': '{ext}'").format(
