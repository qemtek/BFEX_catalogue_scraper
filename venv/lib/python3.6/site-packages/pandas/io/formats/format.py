...
class SeriesFormatter(object):
    ...
    def _get_footer(self):
        ...
        if self.name is not False and name is not None:
            ...
            footer += (
                (u"Name: {sname}".format(sname=series_name)) if name is not None else ""
            )
...
class EastAsianTextAdjustment(TextAdjustment):
    ...
    def len(self, text):
        return compat.east_asian_len(
            text, encoding=self.encoding, ambiguous_width=self.ambiguous_width
        )
...
class EngFormatter(object):
    ...
    def __call__(self, num):
        ...
        if self.accuracy is None:  # pragma: no cover
            ...
            format_str = u("{{mant: .{acc:d}f}}{{prefix}}").format(acc=self.accuracy)
