...
class ClientCreator(object):
    ...
    def _create_client_class(self, service_name, service_model):
        ...
        self._event_emitter.emit(
            "creating-client-class.%s" % service_name,
    ...
    def _set_s3_addressing_style(
        self, endpoint_url, s3_config, event_emitter, partition
    ):
        if s3_config is None:
            s3_config = {}

        addressing_style = self._get_s3_addressing_style(endpoint_url, s3_config)
    ...
    def _get_s3_addressing_handler(
        self, endpoint_url, s3_config, addressing_style, partition
    ):
...
class ClientEndpointBridge(object):
    ...
    def resolve(
        ...
        resolved = self.endpoint_resolver.construct_endpoint(service_name, region_name)
        if resolved:
        ...
        if endpoint_url is None:
            if self._is_s3_dualstack_mode(service_name):
                endpoint_url = self._create_dualstack_endpoint(
                    service_name, region_name, resolved["dnsSuffix"], is_secure
                )
            else:
    ...
    def _is_s3_dualstack_mode(self, service_name):
        ...
        if self.scoped_config is None:
            return False
        enabled = self.scoped_config.get("s3", {}).get("use_dualstack_endpoint", False)
        if enabled in [True, "True", "true"]:
            return True
        return False

    def _create_dualstack_endpoint(
        self, service_name, region_name, dns_suffix, is_secure
    ):
...
class BaseClient(object):
    ...
    def _register_handlers(self):
        ...
        self.meta.events.register(
            "request-created.%s" % self.meta.service_model.endpoint_prefix,
    ...
    def _emit_api_params(self, api_params, operation_model, context):
        ...
        event_name = "before-parameter-build.{endpoint_prefix}.{operation_name}"
        self.meta.events.emit(
            event_name.format(
                endpoint_prefix=self._service_model.endpoint_prefix,
                operation_name=operation_name,
