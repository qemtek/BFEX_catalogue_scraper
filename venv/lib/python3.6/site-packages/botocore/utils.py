    ...
    def _get_request(self, url, timeout, num_attempts=1):
        ...
        for i in range(num_attempts):
            try:
                ...
                logger.debug(
                    "Caught exception while trying to retrieve " "credentials: %s",
                    e,
                    exc_info=True,
                )
            else:
                ...
                for field in fields:
                    if field.endswith("/"):
                        ...
                        val = self._get_request(
                            url + field, timeout=timeout, num_attempts=num_attempts
...
class S3RegionRedirector(object):
    ...
    def redirect_from_error(self, request_dict, response, operation, **kwargs):
        ...
        is_special_head_object = error_code in ["301", "400"] and operation.name in [
            "HeadObject",
            "HeadBucket",
        ]
        ...
        if not any(
            [is_special_head_object, is_wrong_signing_region, is_permanent_redirect]
        ):
            return
...
class ContainerMetadataFetcher(object):
    ...
    def _get_response(self, full_url, headers, timeout):
        try:
            response = self._session.get(full_url, headers=headers, timeout=timeout)
            if response.status_code != 200:
            ...
            try:
                ...
                raise MetadataRetrievalError(
                    error_msg=(
                        "Unable to parse JSON returned from "
