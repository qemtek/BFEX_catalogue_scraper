...
class AWSPreparedRequest(models.PreparedRequest):
    ...
    def __init__(self, original_request):
        ...
        self.hooks.setdefault("response", []).append(self.reset_stream_on_redirect)

    def reset_stream_on_redirect(self, response, **kwargs):
        if response.status_code in REDIRECT_STATI and self._looks_like_file(self.body):
    ...
    def reset_stream(self):
        ...
        if (
            self.body is None
            or isinstance(self.body, six.text_type)
            or isinstance(self.body, six.binary_type)
        ):
            return
        try:
