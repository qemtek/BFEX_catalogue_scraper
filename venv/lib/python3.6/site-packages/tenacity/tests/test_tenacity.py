    ...
    def test_retry_child_class_with_override_backward_compat(self):
        class MyStop(tenacity.stop_after_attempt):
            ...
            def __call__(self, attempt_number, seconds_since_start):
                return super(MyStop, self).__call__(attempt_number, seconds_since_start)
...
class TestWaitConditions(unittest.TestCase):
    ...
    def test_wait_random_exponential_statistically(self):
        ...
        attempt = []
        for i in six.moves.range(10):
            attempt.append([fn(i, 0) for _ in six.moves.range(4000)])
    ...
    def test_wait_backward_compat(self):
        ...
        def wait1(previous_attempt_number, delay_since_first_attempt):
            wait1.calls.append((previous_attempt_number, delay_since_first_attempt))
            return 0

        wait1.calls = []

        def wait2(previous_attempt_number, delay_since_first_attempt, last_result):
            wait2.calls.append(
                (previous_attempt_number, delay_since_first_attempt, last_result)
            )
            return 0
        ...
        self.assertEqual([str(t[2].exception()) for t in wait2.calls], ["Broken"] * 3)
    ...
    def test_wait_class_backward_compatibility(self):
        ...
        self.assertEqual(waitobj(1, 0.1, tenacity.Future.construct(1, 1, False)), 5)
    ...
    def test_wait_retry_state_attributes(self):
        ...
        self.assertGreater(retry_state.outcome_timestamp, retry_state.start_time)
...
class TestRetryConditions(unittest.TestCase):
    ...
    def test_retry_try_again(self):
        self._attempts = 0
        Retrying(stop=tenacity.stop_after_attempt(5), retry=tenacity.retry_never).call(
...
class TestBeforeAfterAttempts(unittest.TestCase):
    ...
    def test_before_sleep_backward_compat(self):
        def _before_sleep(retry_obj, sleep, last_result):
            ...
            _before_sleep.attempt_number = retry_obj.statistics["attempt_number"]
