...
class CharMaps(object):
    ...
    alltt = {ord("\\"): r"\textbackslash{}", ord("{"): r"\{", ord("}"): r"\}"}
...
class LaTeXTranslator(nodes.NodeVisitor):
    ...
    def depart_citation_reference(self, node):
        if self._use_latex_citations:
            ...
            next_siblings = node.traverse(
                descend=False, siblings=True, include_self=False
            )
            if len(next_siblings) > 1:
                next = next_siblings[0]
                if isinstance(next, nodes.Text) and next.astext() in (" ", "\n"):
    ...
    def visit_target(self, node):
        ...
        set_anchor = not (
            isinstance(node.parent, nodes.caption)
            or isinstance(node.parent, nodes.title)
        )
