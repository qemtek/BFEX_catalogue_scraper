...
class StripClassesAndElements(Transform):
    ...
    def apply(self):
        ...
        self.strip_elements = dict(
            [
                (key, None)
                for key in (self.document.settings.strip_elements_with_classes or [])
            ]
        )
        self.strip_classes = dict(
            [(key, None) for key in (self.document.settings.strip_classes or [])]
        )
...
class SmartQuotes(Transform):
    ...
    def get_tokens(self, txtnodes):
        ...
        texttype = {True: "literal", False: "plain"}  # "literal" text is not changed:
