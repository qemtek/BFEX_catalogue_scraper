...
CommandResult = namedtuple("CommandResult", ["num_tasks_failed", "num_tasks_warned"])
...
class ResultPrinter(BaseResultHandler):
    ...
    SUCCESS_FORMAT = u"{transfer_type}: {transfer_location}"
    DRY_RUN_FORMAT = u"(dryrun) " + SUCCESS_FORMAT
    FAILURE_FORMAT = u"{transfer_type} failed: {transfer_location} {exception}"
    ...
    WARNING_FORMAT = u"{message}"
    ERROR_FORMAT = u"fatal error: {exception}"
    ...
    def _print_progress(self, **kwargs):
        ...
        if self._result_recorder.expected_bytes_transferred > 0:
            ...
            transfer_speed = (
                human_readable_size(self._result_recorder.bytes_transfer_speed) + "/s"
            )
