...
class BaseTransferRequestSubmitter(object):
    ...
    def _should_inject_content_type(self):
        return self._cli_params.get("guess_mime_type") and not self._cli_params.get(
            "content_type"
        )
    ...
    def _warn_parent_reference(self, fileinfo):
        ...
        escapes_cwd = os.path.normpath(fileinfo.compare_key).startswith(parent_prefix)
        if escapes_cwd:
            warning = create_warning(
...
class UploadStreamRequestSubmitter(UploadRequestSubmitter):
    ...
    def can_submit(self, fileinfo):
        return fileinfo.operation_name == "upload" and self._cli_params.get("is_stream")
