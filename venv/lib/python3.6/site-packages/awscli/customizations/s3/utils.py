...
class StablePriorityQueue(queue.Queue):
    ...
    def _put(self, item):
        priority = min(
            getattr(item, "PRIORITY", self.default_priority), self.default_priority
        )
...
class BucketLister(object):
    ...
    def list_objects(self, bucket, prefix=None, page_size=None, extra_args=None):
...
class RequestParamsMapper(object):
    ...
    def _set_metadata_directive_param(cls, request_params, cli_params):
        if cli_params.get("metadata_directive"):
            request_params["MetadataDirective"] = cli_params["metadata_directive"]
    ...
    @classmethod
    def _set_sse_c_and_copy_source_request_params(cls, request_params, cli_params):
        cls._set_sse_c_request_params(request_params, cli_params)
    ...
    def on_queued(self, future, **kwargs):
        ...
        try:
            ...
            if not e.errno == errno.EEXIST:
                raise CreateDirectoryError("Could not create directory %s: %s" % (d, e))
