...
from awscli.customizations.s3.utils import find_dest_path_comp_key
...
SUMMARIZE = {
    "name": "summarize",
    "action": "store_true",
    "help_text": ("Displays summary information " "(number of objects, total size)."),
}
...
EXPIRES = {
    "name": "expires",
    "help_text": ("The date and time at which the object is no longer cacheable."),
}
...
METADATA = {
    "name": "metadata",
    "cli_type_name": "map",
    "schema": {"type": "map", "key": {"type": "string"}, "value": {"type": "string"}},
    "help_text": (
...
class ListCommand(S3Command):
    ...
    def _list_all_objects(self, bucket, key, page_size=None, request_payer=None):
...
class WebsiteCommand(S3Command):
    ...
    USAGE = "<S3Uri>"
    ARG_TABLE = [
        {"name": "paths", "nargs": 1, "positional_arg": True, "synopsis": USAGE},
        INDEX_DOCUMENT,
        ERROR_DOCUMENT,
    ]
...
class S3TransferCommand(S3Command):
    def _run_main(self, parsed_args, parsed_globals):
        ...
        cmd_params = CommandParameters(self.NAME, params, self.USAGE)
...
class CpCommand(S3TransferCommand):
    ...
    ARG_TABLE = (
        [{"name": "paths", "nargs": 2, "positional_arg": True, "synopsis": USAGE}]
        + TRANSFER_ARGS
...
class MbCommand(S3Command):
    ...
    def _run_main(self, parsed_args, parsed_globals):
        ...
        try:
            ...
            uni_print("make_bucket failed: %s %s\n" % (parsed_args.path, e), sys.stderr)
            return 1
...
class RbCommand(S3Command):
    ...
    USAGE = "<S3Uri>"
    ARG_TABLE = [{"name": "path", "positional_arg": True, "synopsis": USAGE}, FORCE]
    ...
    def _run_main(self, parsed_args, parsed_globals):
        ...
        if key:
            raise ValueError(
                "Please specify a valid bucket name only." " E.g. s3://%s" % bucket
            )
...
class CommandArchitecture(object):
    ...
    def run(self):
        ...
        fgen_request_parameters = self._get_file_generator_request_parameters_skeleton()
        ...
        rgen_request_parameters = self._get_file_generator_request_parameters_skeleton()
