...
from awscli.customizations.s3.utils import EPOCH_TIME
...
class FileDecodingError(Exception):
    ...
    def __init__(self, directory, filename):
        ...
        self.error_message = (
            ...
            'directory "%s". \n%s' % (repr(self.file_name), self.directory, self.ADVICE)
        )
        ...
        if not self.should_ignore_file(path):
            if not dir_op:
                ...
                for name in listdir_names:
                    if not self.should_ignore_file_with_decoding_warnings(path, name):
