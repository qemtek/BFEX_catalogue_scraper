...
def register(event_handler):
    ...
    event_handler.register(
        ...
        validate_mutually_exclusive_handler(
            ["default_root_object", "origin_domain_name"], ["distribution_config"]
        ),
    )
    event_handler.register(
    ...
    event_handler.register(
        ...
        lambda argument_table, **kwargs: argument_table.__setitem__(
            "default-root-object",
            UpdateDefaultRootObject(context=context, argument_table=argument_table),
        ),
    )
...
class ExclusiveArgument(CustomArgument):
    ...
    def __init__(
        self, name, argument_table, exclusive_to="distribution-config", help_text=""
    ):
...
class OriginDomainName(ExclusiveArgument):
    ...
    def add_to_params(self, parameters, value):
        if value is None:
            return
        parameters.setdefault("DistributionConfig", self.distribution_config_template())
        ...
        parameters["DistributionConfig"]["Origins"] = {"Quantity": 1, "Items": [item]}
...
class SignCommand(BasicCommand):
    ...
    ARG_TABLE = [
        ...
        },
        {
            "name": "date-less-than",
            "required": True,
            "help_text": "The expiration date and time for the URL. " + DATE_FORMAT,
        },
        {
            "name": "date-greater-than",
            "help_text": "An optional start date and time for the URL. " + DATE_FORMAT,
        },
        {
            "name": "ip-address",
            "help_text": (
    ...
    def _run_main(self, args, parsed_globals):
        signer = CloudFrontSigner(args.key_pair_id, RSASigner(args.private_key).sign)
        ...
        if date_greater_than is not None or args.ip_address is not None:
            ...
            sys.stdout.write(signer.generate_presigned_url(args.url, policy=policy))
        else:
            sys.stdout.write(
