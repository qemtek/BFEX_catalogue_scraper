...
from awscli.customizations.datapipeline.createdefaultroles import CreateDefaultRoles
from awscli.customizations.datapipeline.listrunsformatter import ListRunsFormatter
...
def register_customizations(cli):
    ...
    cli.register("after-call.datapipeline.GetPipelineDefinition", translate_definition)
    ...
    while current != "======\nOutput\n======":
        try:
            ...
            raise DocSectionNotFoundError(
                'Could not find the "output" section for the command: %s' % help_command
            )
    doc.write("======\nOutput\n======")
    doc.write(
...
def add_pipeline_definition(argument_table, **kwargs):
    ...
    argument_table["parameter-objects"] = ParameterObjectsArgument(
        "parameter-objects", required=False, help_text=PARAMETER_OBJECTS_HELP_TEXT
    )

    argument_table["parameter-values-uri"] = ParameterValuesArgument(
        "parameter-values-uri", required=False, help_text=PARAMETER_VALUES_HELP_TEXT
    )
...
def convert_described_objects(api_describe_objects, sort_key_func=None):
    ...
    for obj in api_describe_objects:
        new_fields = {"@id": obj["id"], "name": obj["name"]}
        for field in obj["fields"]:
            new_fields[field["key"]] = field.get("stringValue", field.get("refValue"))
...
class QueryArgBuilder(object):
    ...
    def build_query(self, parsed_args):
        selectors = []
        if parsed_args.start_interval is None and parsed_args.schedule_interval is None:
...
class ParameterValuesArgument(CustomArgument):
    def add_to_params(self, parameters, value):
        ...
        if parameters.get("parameterValues", None) is not None:
            raise Exception("Only parameter-values or parameter-values-uri is allowed")
...
class ListRunsCommand(BasicCommand):
    ...
    def _parse_type_args(self, parsed_args):
        ...
        if parsed_args.status is not None:
            parsed_args.status = [arg.strip() for arg in parsed_args.status.split(",")]
    ...
    def _list_runs(self, parsed_args, parsed_globals):
        ...
        converted = convert_described_objects(
            objects,
            sort_key_func=lambda x: (x.get("@scheduledStartTime"), x.get("name")),
        )
