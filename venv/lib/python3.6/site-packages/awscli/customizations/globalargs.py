...
def register_parse_global_args(cli):
    cli.register("top-level-args-parsed", resolve_types, unique_id="resolve-types")
    cli.register("top-level-args-parsed", no_sign_request, unique_id="no-sign")
    cli.register(
        "top-level-args-parsed", resolve_verify_ssl, unique_id="resolve-verify-ssl"
    )
    cli.register(
...
def resolve_verify_ssl(parsed_args, session, **kwargs):
    ...
    if arg_value is not None:
        ...
        if not arg_value:
            ...
            verify = getattr(
                parsed_args, "ca_bundle", None
            ) or session.get_config_variable("ca_bundle")
...
def no_sign_request(parsed_args, session, **kwargs):
    if not parsed_args.sign_request:
        ...
        session.register("choose-signer", disable_signing, unique_id="disable-signing")
