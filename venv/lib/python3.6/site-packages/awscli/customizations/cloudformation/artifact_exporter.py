...
def parse_s3_url(
    url, bucket_name_property="Bucket", object_key_property="Key", version_property=None
):
    ...
    raise ValueError(
        "URL given to the parse method is not a valid S3 url " "{0}".format(url)
    )
...
def upload_local_artifacts(
    ...
    if is_s3_url(local_path):
        ...
        LOG.debug(
            "Property {0} of {1} is already a S3 URL".format(property_name, resource_id)
        )
        return local_path
    ...
    raise exceptions.InvalidLocalPathError(
        resource_id=resource_id, property_name=property_name, local_path=local_path
    )
