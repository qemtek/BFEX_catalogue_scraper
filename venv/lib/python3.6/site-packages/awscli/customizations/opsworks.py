...
class OpsWorksRegister(BasicCommand):
    ...
    def prevalidate_arguments(self, args):
        ...
        if args.local and platform.system() != "Linux":
            raise ValueError("Non-Linux instances are not supported by AWS OpsWorks.")
        ...
        if args.infrastructure_class == "ec2":
            if args.private_ip:
                raise ValueError("--override-private-ip is not supported for EC2.")
            if args.public_ip:
                raise ValueError("--override-public-ip is not supported for EC2.")

        if args.infrastructure_class == "on-premises" and args.use_instance_profile:
            raise ValueError("--use-instance-profile is only supported for EC2.")
    ...
    def retrieve_stack(self, args):
        ...
        self._stack = self.opsworks.describe_stacks(StackIds=[args.stack_id])["Stacks"][
            0
        ]
        self._prov_params = self.opsworks.describe_stack_provisioning_parameters(
        ...
        if args.infrastructure_class == "ec2" and not args.local:
            ...
            ec2 = self._session.create_client("ec2", region_name=self._stack["Region"])
            ...
            if INSTANCE_ID_RE.match(args.target):
                ...
                conditions.append(
                    lambda instance: instance.get("PrivateIpAddress") == args.target
            ...
            if not instances:
                raise ValueError("Did not find any instance matching %s." % args.target)
            elif len(instances) > 1:
                raise ValueError(
                    "Found multiple instances matching %s: %s."
                    % (args.target, ", ".join(i["InstanceId"] for i in instances))
                )
    ...
    def validate_arguments(self, args):
        ...
        if args.hostname:
            ...
            if any(
                args.hostname.lower() == instance["Hostname"] for instance in instances
            ):
                raise ValueError(
        ...
        if args.infrastructure_class == "ec2" and args.local:
            ...
            region = json.loads(ensure_text_type(urlopen(IDENTITY_URL).read()))[
                "region"
            ]
    ...
    def create_iam_entities(self, args):
        ...
        for try_ in range(20):
            ...
            try:
                ...
                if e.response.get("Error", {}).get("Code") == "EntityAlreadyExists":
                    LOG.debug(
                        "IAM user %s already exists, trying another name", username
                    )
        ...
        try:
            self.iam.attach_user_policy(PolicyArn=IAM_POLICY_ARN, UserName=username)
        except ClientError as e:
            if e.response.get("Error", {}).get("Code") == "AccessDenied":
                ...
                LOG.debug("Put policy %s to user %s", IAM_USER_POLICY_NAME, username)
            else:
                raise
        else:
            LOG.debug("Attached policy %s to user %s", IAM_POLICY_ARN, username)

        LOG.debug("Creating an access key")
        self.access_key = self.iam.create_access_key(UserName=username)["AccessKey"]
    ...
    def _pre_config_document(self, args):
        ...
        if self.access_key:
            ...
            parameters["secret_access_key"] = self.access_key["SecretAccessKey"]
    ...
    def _iam_policy_document(arn, timeout=None):
        ...
        if timeout is not None:
            ...
            statement["Condition"] = {
                "DateLessThan": {
                    "aws:CurrentTime": valid_until.strftime("%Y-%m-%dT%H:%M:%SZ")
                }
            }
    ...
    def _to_ruby_yaml(parameters):
        return "\n".join(
            ":%s: %s" % (k, json.dumps(v)) for k, v in sorted(parameters.items())
        )
