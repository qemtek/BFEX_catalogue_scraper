...
class Push(BasicCommand):
    ...
    def _run_main(self, parsed_args, parsed_globals):
        ...
        self.s3 = self._session.create_client("s3", region_name=parsed_globals.region)
        self._push(parsed_args)

    def _validate_args(self, parsed_args):
        validate_s3_location(parsed_args, "s3_location")
        if parsed_args.ignore_hidden_files and parsed_args.no_ignore_hidden_files:
            raise RuntimeError(
    ...
    def _push(self, params):
        with self._compress(params.source, params.ignore_hidden_files) as bundle:
            try:
        ...
        with tempfile.TemporaryFile("w+b") as tf:
            ...
            try:
                ...
                if not contains_appspec:
                    raise RuntimeError("{0} was not found".format(appspec_path))
            finally:
                zf.close()
            yield tf
    ...
    def _upload_to_s3(self, params, bundle):
        ...
        if size_remaining < MULTIPART_LIMIT:
            return self.s3.put_object(Bucket=params.bucket, Key=params.key, Body=bundle)
        else:
