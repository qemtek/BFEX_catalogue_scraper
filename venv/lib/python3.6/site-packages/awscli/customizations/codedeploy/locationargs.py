...
def modify_revision_arguments(argument_table, session, **kwargs):
    ...
    argument_table[GITHUB_LOCATION_ARG_DESCRIPTION["name"]] = GitHubLocationArgument(
        argument_model=github_model, session=session, **GITHUB_LOCATION_ARG_DESCRIPTION
    )
...
class S3LocationArgument(LocationArgument):
    def build_revision_location(self, value_dict):
        ...
        if not all(map(valid, required)):
            raise RuntimeError("--s3-location must specify bucket, key and bundleType.")
        revision = {
            "revisionType": "S3",
            "s3Location": {
