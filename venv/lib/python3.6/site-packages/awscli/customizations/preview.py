...
def mark_as_preview(command_table, session, **kwargs):
    ...
    for preview_service in PREVIEW_SERVICES:
        ...
        if preview_service in allowed_services:
            ...
            # enable this service.
            logger.debug(
                "Preview service enabled through config file: %s", preview_service
            )
            is_enabled = True
        original_command = command_table[preview_service]
        preview_cls = type(
            "PreviewCommand", (PreviewModeCommandMixin, original_command.__class__), {}
        )
        ...
        session.get_component("event_emitter").register_last(
            "doc-description.%s" % preview_service, update_description_with_preview
        )
...
class PreviewModeCommandMixin(object):
    ...
    def __call__(self, args, parsed_globals):
        if self._is_enabled or self._is_help_command(args):
            return super(PreviewModeCommandMixin, self).__call__(args, parsed_globals)
        else:
