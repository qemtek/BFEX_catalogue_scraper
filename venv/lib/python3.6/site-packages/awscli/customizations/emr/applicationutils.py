...
def build_applications(region, parsed_applications, ami_version=None):
    ...
    for app_config in parsed_applications:
        ...
        if app_name == constants.HIVE:
            ...
            ba_list.append(_build_ganglia_install_bootstrap_action(region=region))
        elif app_name == constants.HBASE:
            ba_list.append(_build_hbase_install_bootstrap_action(region=region))
            if ami_version >= "3.0":
                step_list.append(
                    _build_hbase_install_step(constants.HBASE_PATH_HADOOP2_INSTALL_JAR)
                )
            elif ami_version >= "2.1":
                step_list.append(
                    _build_hbase_install_step(constants.HBASE_PATH_HADOOP1_INSTALL_JAR)
                )
            else:
                raise ValueError(
            ...
            app_list.append(
                _build_supported_product(app_config["Name"], app_config.get("Args"))
            )
...
def _build_install_hive_step(region, action_on_failure=constants.TERMINATE_CLUSTER):
    step_args = [
...
def _build_pig_install_step(region, action_on_failure=constants.TERMINATE_CLUSTER):
    step_args = [
