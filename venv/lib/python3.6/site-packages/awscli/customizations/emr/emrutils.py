...
def get_script_runner(region="us-east-1"):
    if region is None:
        region = "us-east-1"
    return build_s3_link(relative_path=constants.SCRIPT_RUNNER_PATH, region=region)


def check_required_field(structure, name, value):
    if not value:
        raise exceptions.MissingParametersError(object_name=structure, missing=name)
...
def call(
    ...
    client = session.create_client(
        "emr", region_name=region_name, endpoint_url=endpoint_url, verify=verify
    )
...
def join(values, separator=",", lastSeparator="and"):
    ...
    if len(values) < 1:
        ...
        return " ".join([separator.join(values[:-1]), lastSeparator, values[-1]])
...
def get_release_label(cluster_id, session, region, endpoint_url, verify_ssl):
    cluster = get_cluster(cluster_id, session, region, endpoint_url, verify_ssl)
