...
class DescribeCluster(Command):
    ...
    def _run_main_command(self, parsed_args, parsed_globals):
        ...
        if "Cluster" in describe_cluster_result:
            ...
            if "Ec2InstanceAttributes" in describe_cluster:
                ec2_instance_attr_keys = describe_cluster[
                    "Ec2InstanceAttributes"
                ].keys()
                ec2_instance_attr = describe_cluster["Ec2InstanceAttributes"]
        else:
            ec2_instance_attr_keys = {}

        if is_fleet_based_cluster:
            list_instance_fleets_result = self._call(
                self._session, "list_instance_fleets", parameters, parsed_globals
            )
        else:
        ...
        emrutils.display_response(
            self._session, "describe_cluster", constructed_result, parsed_globals
        )
    ...
    def _construct_result(
        ...
        if (
            ...
            result["Cluster"]["InstanceFleets"] = list_instance_fleets_result.get(
                "InstanceFleets"
            )
        if (
            ...
            result["Cluster"]["InstanceGroups"] = list_instance_groups_result.get(
                "InstanceGroups"
            )
        if (
            list_bootstrap_actions_result is not None
            and list_bootstrap_actions_result.get("BootstrapActions") is not None
        ):
            result["Cluster"]["BootstrapActions"] = list_bootstrap_actions_result[
                "BootstrapActions"
            ]
