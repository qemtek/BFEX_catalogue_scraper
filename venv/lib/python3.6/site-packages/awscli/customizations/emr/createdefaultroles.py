...
def _get_regex_match_from_endpoint_host(endpoint_host):
    if endpoint_host is None:
        return None
    regex_match = re.match("(https?://)([^.]+).elasticmapreduce.([^/]*)", endpoint_host)
...
class CreateDefaultRoles(Command):
    ...
    def _run_main_command(self, parsed_args, parsed_globals):
        ...
        if self.check_if_instance_profile_exists(instance_profile_name, parsed_globals):
        ...
        emr_autoscaling_result, emr_autoscaling_policy = self._create_role_if_not_exists(
            parsed_globals,
    ...
    def _construct_result(
        ...
        result = []
        self._construct_role_and_role_policy_structure(result, ec2_response, ec2_policy)
        self._construct_role_and_role_policy_structure(result, emr_response, emr_policy)
        self._construct_role_and_role_policy_structure(
            result, emr_autoscaling_response, emr_autoscaling_policy
        )
        return result

    def _construct_role_and_role_policy_structure(self, list, response, policy):
    ...
    def check_if_instance_profile_exists(self, instance_profile_name, parsed_globals):
        parameters = {"InstanceProfileName": instance_profile_name}
        try:
            self._call_iam_operation("GetInstanceProfile", parameters, parsed_globals)
        except botocore.exceptions.ClientError as e:
    ...
    def _create_role_with_role_policy(
        ...
        if len(service_names) == 1:
            service_principal = get_service_principal(
                service_names[0], self.emr_endpoint_url
            )
        else:
            service_principal = []
    ...
    def _create_instance_profile_with_role(
        ...
        parameters = {"InstanceProfileName": instance_profile_name}
        self._call_iam_operation("CreateInstanceProfile", parameters, parsed_globals)
        # Adding the role to the Instance Profile
        parameters = {}
        parameters["InstanceProfileName"] = instance_profile_name
        parameters["RoleName"] = role_name
        self._call_iam_operation("AddRoleToInstanceProfile", parameters, parsed_globals)
