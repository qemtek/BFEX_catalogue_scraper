...
from awscli.customizations.emr.listclusters import modify_list_clusters_argument
...
def emr_initialize(cli):
    ...
    cli.register(
        "building-argument-table.emr.list-clusters", modify_list_clusters_argument
    )
    cli.register(
        "before-building-argument-table-parser.emr.*", override_args_required_option
    )
...
def register_commands(command_table, session, **kwargs):
    ...
    command_table["restore-from-hbase-backup"] = hbase.RestoreFromHBaseBackup(session)
