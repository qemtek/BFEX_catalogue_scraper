...
class CreateCluster(Command):
    ...
    ARG_TABLE = [
        ...
        {"name": "scale-down-behavior", "help_text": helptext.SCALE_DOWN_BEHAVIOR},
        {
        ...
        {"name": "no-enable-debugging", "action": "store_true", "group_name": "debug"},
        {
            "name": "tags",
            "nargs": "+",
        ...
        {"name": "security-configuration", "help_text": helptext.SECURITY_CONFIG},
        {"name": "custom-ami-id", "help_text": helptext.CUSTOM_AMI_ID},
        {"name": "ebs-root-volume-size", "help_text": helptext.EBS_ROOT_VOLUME_SIZE},
        {"name": "repo-upgrade-on-boot", "help_text": helptext.REPO_UPGRADE_ON_BOOT},
        {
    ...
    def _run_main_command(self, parsed_args, parsed_globals):
        ...
        if parsed_args.release_label is not None:
            ...
            if parsed_args.configurations is not None:
                try:
                    params["Configurations"] = json.loads(parsed_args.configurations)
                except ValueError:
                    raise ValueError(
        ...
        if parsed_args.release_label is None and parsed_args.ami_version is not None:
        ...
        if debugging_enabled is True:
            self._update_cluster_dict(
                cluster=params,
                key="Steps",
                value=[self._build_enable_debugging(parsed_args, parsed_globals)],
            )
        ...
        if parsed_args.bootstrap_actions is not None:
            self._build_bootstrap_actions(
                cluster=params, parsed_boostrap_actions=parsed_args.bootstrap_actions
            )
        ...
        if parsed_args.custom_ami_id is not None:
            emrutils.apply_dict(params, "CustomAmiId", parsed_args.custom_ami_id)
    ...
    def _build_bootstrap_actions(self, cluster, parsed_boostrap_actions):
        ...
        for ba in parsed_boostrap_actions:
            ...
            emrutils.apply(
                params=ba_config, key="ScriptBootstrapAction", value=script_arg_config
            )
    ...
    def _validate_release_label_ami_version(self, parsed_args):
        ...
        if parsed_args.ami_version is None and parsed_args.release_label is None:
    ...
    def _validate_required_applications(self, parsed_args):
        ...
        if len(missing_apps) != 0:
            raise exceptions.MissingApplicationsError(applications=missing_apps)

    def _get_missing_applications_for_steps(self, specified_apps, parsed_args):
        allowed_app_steps = set([constants.HIVE, constants.PIG, constants.IMPALA])
        missing_apps = set([])
        if parsed_args.steps is not None:
    ...
    def _filter_configurations_in_special_cases(
        ...
        if parsed_args.use_default_roles:
            configurations = [
                ...
                if x.name is not "service_role" and x.name is not "instance_profile"
            ]
    ...
    def _handle_emrfs_parameters(self, cluster, emrfs_args, release_label):
        if release_label:
            ...
            emrfs_configuration = emrfsutils.build_emrfs_confiuration(emrfs_args)

            self._update_cluster_dict(
                cluster=cluster, key="Configurations", value=[emrfs_configuration]
            )
        else:
            ...
            self._update_cluster_dict(
                cluster=cluster, key="BootstrapActions", value=emrfs_ba_config_list
            )
