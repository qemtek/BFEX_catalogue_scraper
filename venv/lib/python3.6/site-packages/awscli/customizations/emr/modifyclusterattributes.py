...
class ModifyClusterAttr(Command):
    ...
    DESCRIPTION = (
        "Modifies the cluster attributes 'visible-to-all-users' and"
        " 'termination-protected'."
    )
    ARG_TABLE = [
        {"name": "cluster-id", "required": True, "help_text": helptext.CLUSTER_ID},
        {
    ...
    def _run_main_command(self, args, parsed_globals):
        ...
        if args.visible_to_all_users and args.no_visible_to_all_users:
            raise exceptions.MutualExclusiveOptionError(
                option1="--visible-to-all-users", option2="--no-visible-to-all-users"
            )
        if args.termination_protected and args.no_termination_protected:
            raise exceptions.MutualExclusiveOptionError(
                option1="--termination-protected", option2="--no-termination-protected"
            )
        if not (
        ...
        if args.visible_to_all_users or args.no_visible_to_all_users:
            visible = args.visible_to_all_users and not args.no_visible_to_all_users
            parameters = {"JobFlowIds": [args.cluster_id], "VisibleToAllUsers": visible}
            emrutils.call_and_display_response(
                self._session, "SetVisibleToAllUsers", parameters, parsed_globals
            )

        if args.termination_protected or args.no_termination_protected:
            protected = args.termination_protected and not args.no_termination_protected
            parameters = {
                "JobFlowIds": [args.cluster_id],
                "TerminationProtected": protected,
            }
            emrutils.call_and_display_response(
                self._session, "SetTerminationProtection", parameters, parsed_globals
            )
        return 0
