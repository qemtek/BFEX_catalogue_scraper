...
class Formatter(object):
    def __init__(self, output=None, include=None, exclude=None):
        ...
        if include and exclude:
            raise ValueError("Either input or exclude can be provided but not both")
...
class DetailedFormatter(Formatter):
    _SIG_FILTER = RegexFilter("Signature=([a-z0-9]{4})[a-z0-9]{60}", r"Signature=\1...")
    ...
    _SECTIONS = {
        ...
        "HTTP_RESPONSE": {
            ...
            "values": [
                {"description": "with status code", "payload_key": "status_code"},
                {
                    "description": "with headers",
                    "payload_key": "headers",
                    "value_format": "dictionary",
        ...
        "PARSED_RESPONSE": {
            ...
            "values": [{"description": "parsed to", "value_format": "dictionary"}],
        },
        "CLI_RC": {
    ...
    def _format_description(self, value_description):
        return self._color_if_configured(value_description + ": ", "description")
...
class SectionValuePrettyFormatter(object):
    ...
    def _pformat_timestamp(self, event_timestamp, event_record=None):
        return datetime.datetime.fromtimestamp(event_timestamp / 1000.0).strftime(
            "%Y-%m-%d %H:%M:%S.%f"
        )[:-3]
...
class ShowCommand(HistorySubcommand):
    ...
    def _get_formatter(self, parsed_args, parsed_globals, output_stream):
        ...
        if format_type == "detailed":
            formatter_kwargs["colorize"] = self._should_use_color(parsed_globals)
