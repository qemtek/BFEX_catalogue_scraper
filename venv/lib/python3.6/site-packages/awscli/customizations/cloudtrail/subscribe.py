...
class CloudTrailSubscribe(BasicCommand):
    ...
    ARG_TABLE = [
        ...
        {"name": "s3-new-bucket", "help_text": "Create a new S3 bucket with this name"},
        {
            "name": "s3-use-bucket",
            "help_text": "Use an existing S3 bucket with this name",
        },
        {"name": "s3-prefix", "help_text": "S3 object prefix"},
        {"name": "sns-new-topic", "help_text": "Create a new SNS topic with this name"},
        {
            "name": "include-global-service-events",
            "help_text": "Whether to include global service events",
        },
        {"name": "s3-custom-policy", "help_text": "Custom S3 policy template or URL"},
        {"name": "sns-custom-policy", "help_text": "Custom SNS policy template or URL"},
    ]
    ...
    def _call(self, options, parsed_globals):
        ...
        if options.s3_new_bucket:
            ...
            if self.UPDATE and options.s3_prefix is None:
                ...
                res = self.cloudtrail.describe_trails(trailNameList=[options.name])
                trail_info = res["trailList"][0]

                if "S3KeyPrefix" in trail_info:
                    LOG.debug(
                        "Setting S3 prefix to {0}".format(trail_info["S3KeyPrefix"])
                    )
                    options.s3_prefix = trail_info["S3KeyPrefix"]

            self.setup_new_bucket(bucket, options.s3_prefix, options.s3_custom_policy)
        elif not bucket and not self.UPDATE:
            # No bucket was passed for creation.
            raise ValueError(
                "You must pass either --s3-use-bucket or" " --s3-new-bucket to create."
            )
        ...
        try:
            cloudtrail_config = self.upsert_cloudtrail_config(
                options.name, bucket, options.s3_prefix, options.sns_new_topic, gse
            )
        except Exception:
