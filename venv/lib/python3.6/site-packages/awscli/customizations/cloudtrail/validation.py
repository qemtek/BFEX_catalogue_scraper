...
import rsa
from awscli.customizations.cloudtrail.utils import get_account_id_from_arn
...
class InvalidDigestFormat(DigestError):
    ...
    def __init__(self, bucket, key):
        message = "Digest file\ts3://%s/%s\tINVALID: invalid format" % (bucket, key)
...
class DigestProvider(object):
    ...
    def load_digest_keys_in_range(self, bucket, prefix, start_date, end_date):
        ...
        target_end_date = format_date(normalize_date(end_date + timedelta(hours=1)))
    ...
    def _create_digest_key(self, start_date, key_prefix):
        ...
        template = (
            "AWSLogs/{account}/CloudTrail-Digest/{source_region}/"
...
class DigestTraverser(object):
    ...
    def traverse(self, start_date, end_date=None):
        ...
        while key and start_date <= last_start_date:
            try:
                ...
                last_start_date = normalize_date(parse_date(digest["digestStartTime"]))
    ...
    def _load_digests(self, bucket, prefix, start_date, end_date):
        return self.digest_provider.load_digest_keys_in_range(
            bucket=bucket, prefix=prefix, start_date=start_date, end_date=end_date
        )
    ...
    def _load_and_validate_digest(self, public_keys, bucket, key):
        ...
        if fingerprint not in public_keys:
            raise DigestError(
                ...
                % (bucket, key, self.digest_provider.trail_home_region, fingerprint)
            )
    ...
    def _load_public_keys(self, start_date, end_date):
        public_keys = self._public_key_provider.get_public_keys(start_date, end_date)
        if not public_keys:
            raise RuntimeError(
...
class CloudTrailValidateLogs(BasicCommand):
    ...
    def _download_log(self, log):
        ...
        try:
            ...
            if computed_hash != log["hashValue"]:
                ...
                self._valid_logs += 1
                self._write_status(
                    ("Log file\ts3://%s/%s\tvalid" % (log["s3Bucket"], log["s3Object"]))
                )
        except ClientError as e:
            if e.response["Error"]["Code"] != "NoSuchKey":
                raise
    ...
    def _write_ratio(self, valid, invalid, name):
        ...
        if total > 0:
            ...
            if invalid > 0:
                sys.stdout.write(", %d/%d %s files INVALID" % (invalid, total, name))

    def _on_missing_digest(self, bucket, last_key, **kwargs):
        self._invalid_digests += 1
        self._write_status(
            "Digest file\ts3://%s/%s\tINVALID: not found" % (bucket, last_key), True
        )
