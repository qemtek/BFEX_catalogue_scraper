...
from awscli.customizations.ecs import filehelpers
...
class ECSDeploy(BasicCommand):
    ...
    MSG_TASK_DEF_REGISTERED = "Successfully registered new ECS task definition {arn}\n"
    ...
    def _run_main(self, parsed_args, parsed_globals):
        ...
        ecs_client_wrapper = ECSClient(self._session, parsed_args, parsed_globals)

        self.resources = self._get_resource_names(parsed_args, ecs_client_wrapper)
    ...
    def _create_and_wait_for_deployment(self, client, appspec):
        ...
        sys.stdout.write(
            self.MSG_SUCCESS.format(task_def=self.task_def_arn, service=service_name)
        )
        sys.stdout.flush()
...
class CodeDeployer:
    ...
    def create_deployment(self, app_name, deploy_grp_name):
        request_obj = self._get_create_deploy_request(app_name, deploy_grp_name)

        try:
            response = self._client.create_deployment(**request_obj)
        except ClientError as e:
            raise exceptions.ServiceClientError(action="create deployment", error=e)
    ...
    def _show_deploy_wait_msg(self, id, wait_min):
        sys.stdout.write(self.MSG_WAITING.format(deployment_id=id, wait=wait_min))
        sys.stdout.flush()
...
class CodeDeployValidator:
    ...
    def validate_application(self):
        ...
        if self.app_details["application"]["computePlatform"] != "ECS":
            raise exceptions.InvalidPlatformError(resource="Application", name=app_name)
    ...
    def validate_deployment_group(self):
        ...
        if compute_platform != "ECS":
            raise exceptions.InvalidPlatformError(resource="Deployment Group", name=dgp)
...
class ECSClient:
    ...
    def get_service_details(self):
        ...
        try:
            ...
            raise exceptions.ServiceClientError(action="describe ECS service", error=e)
