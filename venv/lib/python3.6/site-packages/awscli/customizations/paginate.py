...
def get_paginator_config(session, service_name, operation_name):
    try:
        ...
        return None
    try:
        operation_paginator_config = paginator_model.get_paginator(operation_name)
    except ValueError:
        return None
...
class PageArgument(BaseCLIArgument):
    type_map = {"string": str, "integer": int, "long": int}
    ...
    def add_to_parser(self, parser):
        parser.add_argument(
            self.cli_name, dest=self.py_name, type=self.type_map[self._parse_type]
        )
