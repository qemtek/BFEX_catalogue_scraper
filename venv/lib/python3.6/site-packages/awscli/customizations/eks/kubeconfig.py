...
class Kubeconfig(object):
    ...
    def has_cluster(self, name):
        ...
        if "clusters" not in self.content:
            return False
        return name in [cluster["name"] for cluster in self.content["clusters"]]
    ...
    def validate_config(self, config):
        ...
        if not isinstance(config, Kubeconfig):
            raise KubeconfigCorruptedError(
                "Internal error: " "Not a Kubeconfig object."
            )
        ...
        for key, value in self._validation_content.items():
            if (
                ...
                raise KubeconfigCorruptedError(
                    "{0} is wrong type:{1} "
                    "(Should be {2})".format(
                        key, type(config.content[key]), type(value)
                    )
                )
    ...
    def insert_entry(self, config, key, entry):
        ...
        if not isinstance(array, list):
            raise KubeconfigError(
                "Tried to insert into {0},"
                "which is a {1} "
                "not a {2}".format(key, type(array), list)
            )
        found = False
        for counter, existing_entry in enumerate(array):
            if (
                "name" in existing_entry
                and "name" in entry
    ...
    def _make_context(self, cluster, user, alias=None):
        ...
        return OrderedDict(
            [
                (
                    "context",
                    OrderedDict([("cluster", cluster["name"]), ("user", user["name"])]),
                ),
