...
from awscli.customizations.eks.kubeconfig import KubeconfigAppender
from awscli.customizations.eks.kubeconfig import KubeconfigError
from awscli.customizations.eks.kubeconfig import KubeconfigLoader
from awscli.customizations.eks.kubeconfig import KubeconfigValidator
from awscli.customizations.eks.kubeconfig import KubeconfigWriter
...
class UpdateKubeconfigCommand(BasicCommand):
    ...
    DESCRIPTION = BasicCommand.FROM_FILE("eks", "update-kubeconfig", "_description.rst")
    ...
    def _run_main(self, parsed_args, parsed_globals):
        client = EKSClient(
            self._session, parsed_args.name, parsed_args.role_arn, parsed_globals
        )
...
class EKSClient(object):
    ...
    def get_cluster_entry(self):
        ...
        cert_data = self._get_cluster_description().get(
    ...
    def get_user_entry(self):
        ...
        if self._role_arn is not None:
            generated_user["user"]["exec"]["args"].extend(["--role", self._role_arn])

        if self._session.profile:
            generated_user["user"]["exec"]["env"] = [
                OrderedDict([("name", "AWS_PROFILE"), ("value", self._session.profile)])
            ]
