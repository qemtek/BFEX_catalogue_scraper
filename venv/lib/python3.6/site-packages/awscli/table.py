...
def center_text(text, length=80, left_edge="|", right_edge="|", text_length=None):
...
def align_left(
    text, length, left_edge="|", right_edge="|", text_length=None, left_padding=2
):
    """Left align text."""
    # postcondition: get_text_length(returned_text) == length
    if text_length is None:
...
class ColorizedStyler(Styler):
    ...
    def style_row_element(self, text):
        return (
            colorama.Style.BRIGHT + colorama.Fore.BLUE + text + colorama.Style.RESET_ALL
        )

    def style_indentation_char(self, text):
        return (
            colorama.Style.DIM + colorama.Fore.YELLOW + text + colorama.Style.RESET_ALL
        )
...
class MultiTable(object):
    ...
    def _calculate_max_width(self):
        max_width = max(
            s.total_width(padding=4, with_border=True, outer_padding=s.indent_level)
    ...
    def _render_title(self, section, max_width, stream):
        ...
        if section.title:
            ...
            stream.write(
                center_text(title, max_width, "|", "|", get_text_length(section.title))
                + "\n"
            )
    ...
    def _render_column_titles(self, section, max_width, stream):
        ...
        widths = section.calculate_column_widths(padding=4, max_width=max_width)
