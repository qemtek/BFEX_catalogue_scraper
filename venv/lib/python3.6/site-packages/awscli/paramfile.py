...
def register_uri_param_handler(session, **kwargs):
    ...
    try:
        fetch_url = (
            session.get_scoped_config().get("cli_follow_urlparam", "true") == "true"
        )
...
def get_file(prefix, path, mode):
    ...
    try:
        ...
        raise ResourceLoadingError("Unable to load paramfile %s: %s" % (path, e))
...
def get_uri(prefix, uri):
    try:
        ...
        if r.status_code == 200:
            ...
            raise ResourceLoadingError(
                "received non 200 status code of %s" % (r.status_code)
            )
...
REMOTE_PREFIX_MAP = {"http://": (get_uri, {}), "https://": (get_uri, {})}
