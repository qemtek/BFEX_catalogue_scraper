...
from awscli.table import ColorizedStyler
from awscli.table import MultiTable
from awscli.table import Styler
...
class JSONFormatter(FullyBufferedFormatter):
    def _format_response(self, command_name, response, stream):
        ...
        if response != {}:
            json.dump(
                response, stream, indent=4, default=json_encoder, ensure_ascii=False
            )
            stream.write("\n")
...
class TableFormatter(FullyBufferedFormatter):
    ...
    def __init__(self, args, table=None):
        ...
        if args.color == "auto":
            self.table = MultiTable(initial_section=False, column_separator="|")
        elif args.color == "off":
            styler = Styler()
        ...
        for element in current:
            ...
            for remaining in more:
                ...
                if remaining in element:
                    self._build_table(
                        remaining, element[remaining], indent_level=indent_level + 1
                    )
            ...
            if is_response_paginated(response):
                ...
                for i, page in enumerate(response):
                    ...
                    for result_key in result_keys:
                        ...
                        set_value_from_jmespath(current, result_key.expression, data)
