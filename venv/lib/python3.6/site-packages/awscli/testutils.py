...
class BaseAWSCommandParamsTest(unittest.TestCase):
    ...
    def assert_params_for_cmd(
        self, cmd, params=None, expected_rc=0, stderr_contains=None, ignore_params=None
    ):
...
def aws(
    ...
    if input_file is None:
        input_file = PIPE
    process = Popen(
        full_command, stdout=PIPE, stderr=PIPE, stdin=input_file, shell=True, env=env
    )
...
def _wait_and_collect_mem(process):
    ...
    if platform.system() == "Darwin":
        ...
        raise ValueError(
            "Can't collect memory for process on platform %s." % platform.system()
        )
    memory = []
...
class BaseS3CLICommand(unittest.TestCase):
    ...
    def put_object(self, bucket_name, key_name, contents="", extra_args=None):
        ...
        self.wait_until_key_exists(
            bucket_name, key_name, extra_params=extra_head_params
        )
        return response
    ...
    def _wait_for_key(
        self, bucket_name, key_name, extra_params=None, min_successes=3, exists=True
    ):
        client = self.create_client_for_bucket(bucket_name)
        if exists:
    ...
    def assert_no_errors(self, p):
        self.assertEqual(
            p.rc, 0, "Non zero rc (%s) received: %s" % (p.rc, p.stdout + p.stderr)
        )
