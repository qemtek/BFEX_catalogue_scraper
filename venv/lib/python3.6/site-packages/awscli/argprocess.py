...
from awscli import COMPLEX_TYPES
...
class ParamUnknownKeyError(Exception):
    def __init__(self, key, valid_keys):
        ...
        full_message = "Unknown key '%s', valid choices " "are: %s" % (key, valid_keys)
...
def _detect_shape_structure(param, stack):
    ...
    try:
        if param.type_name in SCALAR_TYPES:
            ...
            sub_types = [
                _detect_shape_structure(p, stack) for p in param.members.values()
            ]
...
def _unpack_json_cli_arg(argument_model, value, cli_name):
    try:
        ...
        raise ParamError(cli_name, "Invalid JSON: %s\nJSON received: %s" % (e, value))
...
def unpack_scalar_cli_arg(argument_model, value, cli_name=""):
    ...
    if argument_model.type_name == "integer" or argument_model.type_name == "long":
        ...
        return float(value)
    elif argument_model.type_name == "blob" and argument_model.serialization.get(
        "streaming"
    ):
...
class ParamShorthandParser(ParamShorthand):
    ...
    def _parse_as_shorthand(self, cli_argument, value, service_id, operation_name):
        try:
    ...
    def _handle_special_cases(self, cli_argument, value, service_id, operation_name):
    ...
    def _should_parse_as_shorthand(self, cli_argument, value):
        ...
        if isinstance(check_val, six.string_types) and check_val.strip().startswith(
            ...
            LOG.debug(
                "Param %s looks like JSON, not considered for " "param shorthand.",
...
class ParamShorthandDocGen(ParamShorthand):
    ...
    def generate_shorthand_example(self, cli_argument, service_id, operation_name):
        ...
        """
        docstring = self._handle_special_cases(cli_argument, service_id, operation_name)
