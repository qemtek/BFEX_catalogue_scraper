...
class CLIDriver(object):
    ...
    def _build_builtin_commands(self, session):
        ...
        for service_name in services:
            commands[service_name] = ServiceCommand(
                cli_name=service_name, session=self.session, service_name=service_name
            )
        return commands
    ...
    def _build_argument_table(self):
        ...
        self.session.emit("building-top-level-params", argument_table=argument_table)
    ...
    def main(self, args=None):
        ...
        try:
            ...
            HISTORY_RECORDER.record("CLI_VERSION", self.session.user_agent(), "CLI")
            ...
            msg = "%s. You can configure credentials by running " '"aws configure".' % e
            self._show_error(msg)
            return 255
    ...
    def _emit_session_event(self, parsed_args):
        ...
        self.session.emit(
            "session-initialized", session=self.session, parsed_args=parsed_args
        )
...
class ServiceCommand(CLICommand):
    ...
    def _create_parser(self):
        ...
        # Also add a 'help' command.
        command_table["help"] = self.create_help_command()
        return ServiceArgParser(operations_table=command_table, service_name=self._name)
...
class ServiceOperation(object):
    ...
    ARG_TYPES = {"list": ListArgument, "boolean": BooleanArgument}
    ...
    def __call__(self, args, parsed_globals):
        ...
        self._emit(
            event, argument_table=self.arg_table, args=args, session=self._session
        )
        ...
        if remaining:
            raise UnknownArgumentError("Unknown options: %s" % ", ".join(remaining))
        event = "operation-args-parsed.%s.%s" % (self._parent_name, self._name)
        self._emit(event, parsed_args=parsed_args, parsed_globals=parsed_globals)
        call_parameters = self._build_call_parameters(parsed_args, self.arg_table)
    ...
    def _create_argument_table(self):
        ...
        for arg_name, arg_shape in arg_dict.items():
            ...
            arg_class = self.ARG_TYPES.get(arg_shape.type_name, self.DEFAULT_ARG_CLASS)
...
class CLIOperationCaller(object):
    ...
    def _make_client_call(self, client, operation_name, parameters, parsed_globals):
