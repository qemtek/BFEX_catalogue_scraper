...
class WindowsHelpRenderer(PagingHelpRenderer):
    ...
    def _convert_doc_content(self, contents):
        text_output = publish_string(contents, writer=TextWriter())
        return text_output
...
class ProviderHelpCommand(HelpCommand):
    ...
    def __init__(self, session, command_table, arg_table, description, synopsis, usage):
...
class ServiceHelpCommand(HelpCommand):
    ...
    def __init__(self, session, obj, command_table, arg_table, name, event_class):
        super(ServiceHelpCommand, self).__init__(session, obj, command_table, arg_table)
        self._name = name
