...
M   33  21.99
    ...
    def test_autonames_and_usecols(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            warnings.filterwarnings("always", "", np.VisibleDeprecationWarning)
            test = np.ndfromtxt(data, usecols=("A", "C", "D"), names=True, dtype=None)
            assert_(w[0].category is np.VisibleDeprecationWarning)
        control = np.array(
    ...
    def test_user_missing_values(self):
        ...
        #
        test = np.mafromtxt(TextIO(data), missing_values="N/A", **basekwargs)
        control = ma.array(
        ...
        test = np.mafromtxt(
            TextIO(data), missing_values={0: -9, 1: -99, 2: -999j}, **basekwargs
        )
        control = ma.array(
            [(0, 0.0, 0j), (1, -999, 1j), (-9, 2.2, -999j), (3, -99, 3j)],
        ...
        #
        test = np.mafromtxt(
            TextIO(data), missing_values={0: -9, "B": -99, "C": -999j}, **basekwargs
        )
        control = ma.array(
            [(0, 0.0, 0j), (1, -999, 1j), (-9, 2.2, -999j), (3, -99, 3j)],
            mask=[(0, 0, 0), (0, 1, 0), (1, 0, 1), (0, 1, 0)],
    ...
    def test_withmissing_float(self):
        data = TextIO("A,B\n0,1.5\n2,-999.00")
        test = np.mafromtxt(
            data, dtype=None, delimiter=",", missing_values="-999.0", names=True
        )
        control = ma.array(
            [(0, 1.5), (2, -1.0)],
            mask=[(False, False), (False, True)],
    ...
    def test_invalid_raise(self):
        ...
        #
        mdata.seek(0)
        assert_raises(ValueError, np.ndfromtxt, mdata, delimiter=",", names=True)
    ...
    def test_single_dtype_w_explicit_names(self):
        ...
        data = "0, 1, 2.3\n4, 5, 6.7"
        mtest = np.ndfromtxt(TextIO(data), delimiter=",", dtype=float, names="a, b, c")
        ctrl = np.array(
    ...
    def test_single_dtype_w_implicit_names(self):
        ...
        data = "a, b, c\n0, 1, 2.3\n4, 5, 6.7"
        mtest = np.ndfromtxt(TextIO(data), delimiter=",", dtype=float, names=True)
        ctrl = np.array(
            [(0.0, 1.0, 2.3), (4.0, 5.0, 6.7)], dtype=[(_, float) for _ in "abc"]
        )
        assert_equal(mtest, ctrl)
